...
//	ARCEMU_INLINE CircularBuffer& GetWriteBuffer() { return writeBuffer; }
	ARCEMU_INLINE StraightBuffer& GetReadBuffer() { return readBuffer; }
	ARCEMU_INLINE StraightBuffer& GetWriteBuffer() { return writeBuffer; }
...
/* Deletion */
	...
	uint8 m_empty_reads;	//number of times nothing was read and waiting timed out
protected:
...
//!!!!!!!!! this needs to be larger then 2 * SOCKET_EVENT_WAIT_TIMOUT !!!!!
...
//					|| i2->first->GetFd() //sockets that never manage to connect anywhere will probably have this non 0. Destructor should handle closing the handle
					...
					)
				{
					...
					{
						i2->second = UNIXTIME + SOCKET_GC_TIMEOUT;
						continue; 
					}
				}
				delete i2->first;
