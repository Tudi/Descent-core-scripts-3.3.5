...
#endif
		memset( _array[Entry],0, sizeof( T ) ); //set those not yet loaded fields to 0 to avoid crashes
		return _array[Entry];
	}
...
public:
	...
	T * AllocateEntry(uint32 Entry)
	{
		...
		T * n = new T;
#endif
//		memset(n, 0, sizeof( T ) ); //set those not yet loaded fields to 0 to avoid crashes
...
public:
	...
	{
		_indexName = strdup_local(IndexName);
		_formatString = strdup_local(FormatString);
	}
	...
	ARCEMU_INLINE void LoadBlock(Field * fields, T * Allocated)
	{
		...
		for(; *p != 0; ++p, ++f)
		{
			...
			case 's':	// Null-terminated string
				*(char**)&structpointer[offset] = strdup_local(f->GetString());
				offset += sizeof(char*);
				break;
