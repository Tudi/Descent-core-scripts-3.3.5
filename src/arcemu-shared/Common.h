...
#define REPACK "E-Muuuuu repack"
#define REPACK_AUTHOR "AvgJoe"
#define REPACK_WEBSITE "nobody@nowhere"
...
struct WayPoint
{
	...
	uint32 id; //this is SQL id and not real wp id
	float x;
	float y;
	float z;
	float o;
...
typedef SimpleVect<WayPoint*> WayPointVect;
...
inline static float CalculateDistance(float x1, float y1, float z1, float x2, float y2, float z2)
{
	float dx = x1 - x2;
	float dy = y1 - y2;
	float dz = z1 - z2;
...
inline unsigned int HexCharToDec( char in )
{
	switch( in )
	{
		case '0' : return 0; 
...
inline unsigned int ByteHexStrToDec(char *input)
{
	return ( HexCharToDec( input[ 1 ] ) + 16 * HexCharToDec( input[ 0 ] ) );
}
// warning, by enabling this define you are aware that you are only delaying the inevitable
// some crashes are not recoverable and those will stack up in time and lead to a full crash
// enabling this define will make windows servers shut down only the map instance where the crash ocured
