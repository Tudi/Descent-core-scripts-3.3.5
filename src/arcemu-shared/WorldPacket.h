...
#define sStackWolrdPacket(name, opcode, size) uint8 local_stackbuffer[ size ]; StackWorldPacket name(opcode, local_stackbuffer, size);
...
//		m_readPos = 0;
	...
	ARCEMU_INLINE void AdvanceWrite(uint32 size )
	{
		m_writePos += size;
	}
...
/*	template<typename T>
	...
	template<typename T>
		void Write(T data)
	{
		while(m_writePos + sizeof(T) >= m_space)
		{
