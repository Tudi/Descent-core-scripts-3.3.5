...
void Arena::OnStart()
{
	...
	sEventMgr.AddEvent(static_cast<CBattleground*>(this), &CBattleground::HookOnShadowSight, EVENT_ARENA_SHADOW_SIGHT, 90000, 1,0);
}
	...
	for(int i = 0; i < 2; i++)
	{
		...
		for(; itr != m_players[i].end(); itr++)
		{
			...
			if (plr != NULL)
			{
				if( m_teams[i] != NULL )
				{
					plr->Event_Achiement_Received(ACHIEVEMENT_CRITERIA_TYPE_PLAY_ARENA,plr->GetMapId(),ACHIEVEMENT_UNUSED_FIELD_VALUE,1,ACHIEVEMENT_EVENT_ACTION_ADD);
					plr->Event_Achiement_Received(ACHIEVEMENT_CRITERIA_TYPE_REACH_TEAM_RATING,m_teams[i]->m_type,ACHIEVEMENT_UNUSED_FIELD_VALUE,m_teams[i]->m_stat_rating,ACHIEVEMENT_EVENT_ACTION_SET_MAX);
				}
				if( victorious )
				{
					plr->Event_Achiement_Received(ACHIEVEMENT_CRITERIA_TYPE_WIN_ARENA,plr->GetMapId(),ACHIEVEMENT_UNUSED_FIELD_VALUE,1,ACHIEVEMENT_EVENT_ACTION_ADD);
					//there is a special filter here for "in row" wins
					if( rated_match )
						plr->Event_Achiement_Received(ACHIEVEMENT_CRITERIA_TYPE_WIN_RATED_ARENA,ACHIEVEMENT_UNUSED_FIELD_VALUE,ACHIEVEMENT_UNUSED_FIELD_VALUE,1,ACHIEVEMENT_EVENT_ACTION_ADD);
				}
