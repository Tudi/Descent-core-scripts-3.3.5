...
void WorldSession::HandleGMTicketCreateOpcode(WorldPacket & recv_data)
{
	...
	sStackWolrdPacket( data, SMSG_GMTICKET_CREATE, 10 );
...
{
	std::string message = "";
	sStackWolrdPacket( data,SMSG_GMTICKET_UPDATETEXT, 10);
...
void WorldSession::HandleGMTicketDeleteOpcode(WorldPacket & recv_data)
{
	...
	// Response - no errors
	sStackWolrdPacket( data, SMSG_GMTICKET_DELETETICKET, 10);
	data << uint32(9);
	SendPacket(&data);
...
{
	sStackWolrdPacket( data, SMSG_GMTICKET_GETTICKET, 800);
	// no data
...
{
	sStackWolrdPacket( data, SMSG_GMTICKET_SYSTEMSTATUS, 10);
