...
//			SpellEntry * loser_spell = dbcSpell.LookupEntry(24950);
			...
			for(uint32 i = 0; i < 2; ++i)
			{
				...
				{
					(*itr)->Root();
					(*itr)->Event_Achiement_Received(ACHIEVEMENT_CRITERIA_TYPE_COMPLETE_BATTLEGROUND,(*itr)->GetMapId(),ACHIEVEMENT_UNUSED_FIELD_VALUE,1,ACHIEVEMENT_EVENT_ACTION_ADD);
					if(i == m_winningteam)
					{
						(*itr)->Event_Achiement_Received(ACHIEVEMENT_CRITERIA_TYPE_WIN_BG,(*itr)->GetMapId(),ACHIEVEMENT_UNUSED_FIELD_VALUE,1,ACHIEVEMENT_EVENT_ACTION_ADD);
...
void WarsongGulch::OnCreate()
{
	...
	// Alliance Gates
	GameObject *gate = SpawnGameObject(179921, 489, 1471.554688f, 1458.778076f, 362.633240f, 0, GAMEOBJECT_CLICKABLE|GAMEOBJECT_UNCLICKABLE, 114, 2.33271f);
	gate->SetByte(GAMEOBJECT_BYTES_1, GAMEOBJECT_BYTES_ANIMPROGRESS, 0xFF);
	...
	// Horde Gates
	gate = SpawnGameObject(179916, 489, 949.1663208f, 1423.7717285f, 345.6241455f, -0.5756807f, GAMEOBJECT_CLICKABLE, 114, 0.900901f);
	gate->SetFloatValue(GAMEOBJECT_PARENTROTATION,-0.0167336f);
	gate->SetFloatValue(GAMEOBJECT_PARENTROTATION_1,-0.004956f);
	gate->SetFloatValue(GAMEOBJECT_PARENTROTATION_2,-0.283972f);
	gate->SetFloatValue(GAMEOBJECT_PARENTROTATION_3,0.9586736f);
	gate->SetByte(GAMEOBJECT_BYTES_1, GAMEOBJECT_BYTES_ANIMPROGRESS, 0xFF);
	gate->SetUInt32Value(GAMEOBJECT_DYNAMIC, 4294901760);
	gate->PushToWorld(m_mapMgr);
	m_gates.push_back(gate);

	gate = SpawnGameObject(179917, 489, 953.0507202f, 1459.8424072f, 340.6525573f, -1.9966197f, GAMEOBJECT_CLICKABLE, 114, 0.854700f);   
	gate->SetFloatValue(GAMEOBJECT_PARENTROTATION,-0.1971825f);
	gate->SetFloatValue(GAMEOBJECT_PARENTROTATION_1,0.1575096f);
	gate->SetFloatValue(GAMEOBJECT_PARENTROTATION_2,-0.8239487f);
	gate->SetFloatValue(GAMEOBJECT_PARENTROTATION_3,0.5073640f);
	gate->SetByte(GAMEOBJECT_BYTES_1, GAMEOBJECT_BYTES_ANIMPROGRESS, 0xFF);
	gate->SetUInt32Value(GAMEOBJECT_DYNAMIC, 4294901760);
	gate->PushToWorld(m_mapMgr);
	m_gates.push_back(gate);
