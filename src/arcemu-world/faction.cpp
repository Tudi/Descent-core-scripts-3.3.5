...
bool isHostile(Object* objA, Object* objB)// B is hostile for A?
{
	...
	if( objA->IsPlayer() )
	{
		...
		if( ( atA && atA->AreaFlags & AREA_SANCTUARY) )
			return false;
	...
	if( objB->IsPlayer() )
	{
		...
		if( atB && ( atB->AreaFlags & AREA_SANCTUARY ) )
			return false;
	}
...
//		|| objB->m_factionDBC == NULL || objA->m_factionDBC == NULL
	...
	if( objA->IsUnit() && objB->IsUnit() )
	{
		...
		if( topA && topB )
		{
			...
			if( topA->IsFFAPvPFlagged() && topB->IsFFAPvPFlagged() )
			{
			...
			if( topA->IsPlayer() && topB->IsPlayer() )
			{
				...
				if( atA->AreaFlags & AREA_SANCTUARY || atB->AreaFlags & AREA_SANCTUARY )
					return false;
			}
		}
		else 
