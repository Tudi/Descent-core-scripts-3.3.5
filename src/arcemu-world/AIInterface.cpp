...
//	m_assistTargets.clear();
	...
	m_returnX = m_returnY = m_returnZ = 0;
	m_last_target_x = m_last_target_y = 0;
	m_forced_target = 0;
	agro_range_mod = 1.0f;
...
void AIInterface::Init(Unit *un, AIType at, MovementType mt, Unit *owner)
{
	...
	if( m_PetOwner )
	{
		if( m_PetOwner->IsPvPFlagged() )
			m_Unit->SetPvPFlag();
		if( m_PetOwner->IsFFAPvPFlagged() )
...
/*					if( SafeCreatureCast( m_Unit )->m_spawn && ( SafeCreatureCast( m_Unit )->m_spawn->channel_target_go || SafeCreatureCast( m_Unit )->m_spawn->channel_target_creature))
				...
				//do not move return position in case we enter combat multiple times without return
				if( m_returnX == 0 )
				{
	...
	if(m_AIType == AITYPE_TOTEM)
	{
		...
		if(p_time >= m_totemspelltimer)
		{
				...
				if( totemspell->spells[i] )
			{
				...
				if( ( totemspell->spells[i]->Effect[0] == SPELL_EFFECT_APPLY_AREA_AURA || totemspell->spells[i]->Effect[0] == SPELL_EFFECT_APPLY_AREA_AURA2
					|| totemspell->spells[i]->Effect[1] == SPELL_EFFECT_APPLY_AREA_AURA || totemspell->spells[i]->Effect[1] == SPELL_EFFECT_APPLY_AREA_AURA2
					|| totemspell->spells[i]->Effect[2] == SPELL_EFFECT_APPLY_AREA_AURA || totemspell->spells[i]->Effect[2] == SPELL_EFFECT_APPLY_AREA_AURA2 ) 
				...
				{
					m_totemspelltimer = MOB_SPELLCAST_GLOBAL_COOLDOWN;
		...
		if(m_AIType == AITYPE_PET )
		{
			if(!m_Unit->bInvincible && m_Unit->IsPet()) 
			{
				...
				if(pPet->GetPetAction() == PET_ACTION_ATTACK || pPet->GetPetState() != PET_STATE_PASSIVE)
				{
					...
					if( GetNextTarget() && isAttackable( pPet, GetNextTarget() ) == false )
					{
...
//		m_updateTargets = false;
		...
		else if( m_aiTargets.size() == 0 && (m_AIType == AITYPE_PET && (m_Unit->IsPet() && SafePetCast(m_Unit)->GetPetState() == PET_STATE_AGGRESSIVE) || (!m_Unit->IsPet() && disable_melee == false ) ) )
		{
		...
		case AGENT_MELEE:
			{
				...
				combatReach[0] = local_target->GetModelHalfSize();
				combatReach[1] = _CalcCombatRange( local_target , false);
					...
					if(m_Unit->isAttackReady(false) && !m_fleeTimer)
					{
						...
						if(infront)
						{
							...
							if(local_target && m_Unit->m_factionDBC &&
								...
								&& local_target->IsPlayer() && !m_Unit->IsPet() && health_before_strike>local_target->GetUInt32Value(UNIT_FIELD_HEALTH)
								&& Rand(m_Unit->get_chance_to_daze( local_target )))
							{
								...
								if(fabs(our_facing-his_facing)<CREATURE_DAZE_TRIGGER_ANGLE && !local_target->HasNegativeAura(CREATURE_SPELL_TO_DAZE))
								{
		...
		case AGENT_SPELL:
			{
				...
				Unit *deafult_target = FindTargetForSpellTargetType( m_nextSpell );
...
//					&& distance >= m_nextSpell->minrange 
							...
							|| m_nextSpell->maxrange == 0) ) // Target is in Range -> Attack
				{
					...
					//global coolwon
					uint32 spell_global_cooldown = MAX( MOB_SPELLCAST_GLOBAL_COOLDOWN, m_nextSpell->global_cooldown );
					next_spell_time = getMSTime() + spell_global_cooldown;
					...
					case TTYPE_SOURCE:
						{
							m_Unit->CastSpellAoF( deafult_target->GetPositionX(),deafult_target->GetPositionY(),deafult_target->GetPositionZ(), spellInfo, true);
							break;
						}
					case TTYPE_DESTINATION:
						{
							m_Unit->CastSpellAoF( m_Unit->GetPositionX(),m_Unit->GetPositionY(),m_Unit->GetPositionZ(), spellInfo, true);
							break;
						}
					default:
						if( m_nextSpell->shout_before_cast )
							m_Unit->SendChatMessage( CHAT_MSG_MONSTER_SAY, LANG_COMMON, m_nextSpell->shout_before_cast );
...
//				m_Unit->SetUInt64Value(UNIT_FIELD_TARGET, 0);
	...
	else if( !local_target || local_target->GetInstanceID() != m_Unit->GetInstanceID() || !local_target->isAlive() || !cansee )
	{
		SetNextTarget( (Unit*)NULL );
		HandleEvent( EVENT_LEAVECOMBAT, m_Unit, 0);
		local_target = GetNextTarget();
...
/*	if( sWorld.Collision && pUnit->IsPlayer() )
	...
	if (pUnit->GetTypeId() == TYPEID_PLAYER && SafePlayerCast(pUnit)->GetMisdirectionTarget() != 0 && m_Unit->GetMapMgr() )
	{
...
EC AF 80 00 
*/
	sStackWolrdPacket( data, SMSG_MONSTER_MOVE, 150);
	...
	if( m_Unit->GetTypeId() == TYPEID_PLAYER )
		SafePlayerCast(m_Unit)->GetSession()->SendPacket(&data);
...
bool AIInterface::StopMovement(uint32 time)
{
	...
	sStackWolrdPacket(data,SMSG_MONSTER_MOVE,30);
	data << m_Unit->GetNewGUID();
	data << uint8(0);
	data << m_Unit->GetPositionX() << m_Unit->GetPositionY() << m_Unit->GetPositionZ();
	data << getMSTime();
...
//		moveTime = (uint32) (distance * 1000 / UnitToFollow->m_runSpeed ); //i wonder if runpeed can ever frop to 0
	...
	//check if we returned to the position we started combat
	if( m_returnX && abs( m_returnX - m_destinationX ) < DISTANCE_TO_SMALL_TO_WALK && abs( m_returnY - m_destinationY ) < DISTANCE_TO_SMALL_TO_WALK )
	{
		m_returnX = 0.0f;
		m_returnY = 0.0f;
		m_returnZ = 0.0f;
...
//		&& m_destinationY == 0.0f && m_destinationZ == 0.0f
	...
	uint32 flags = 0; //see parser step 8 for proper flag descriptions here
	...
	*splineBuf << float(1) << float(1);
	*splineBuf << uint32(0) << uint32(0);
...
//			pWayPoint->SetUInt32Value(UNIT_FIELD_AURA+32, 8326); //invisable & deathworld look
		...
		//root the object
		sStackWolrdPacket(data1,SMSG_FORCE_MOVE_ROOT,10);
...
/*				if (sWorld.Collision)
					...
					if( m_Unit->GetMapMgr() != NULL )
					{
						if(m_moveFly != true)
						{
							...
							if ( target_land_z == NO_WMO_HEIGHT )
							{
								...
								if ( target_land_z == WMO_VMAP_VALUE_NOT_INITIALIZED )
									target_land_z = z;
							}
						}
	...
	if(m_AIState == STATE_FEAR && UnitToFear != NULL && m_creatureState == STOPPED)
	{
		if(getMSTime() > m_FearTimer)   // Wait at point for x ms ;)
		{
			...
			// Calculate new angle to target.
				...
				Fo += (float)fAngleAdd;
				Fx = m_Unit->GetPositionX() + (RandomFloat(20.f)+10.0f)*cosf(Fo);
				Fy = m_Unit->GetPositionY() + (RandomFloat(20.f)+10.0f)*sinf(Fo);
			}
			else
			{
	...
	if(m_AIState == STATE_WANDER && m_creatureState == STOPPED)
	{
		...
		float wanderD = RandomFloat(5.0f) + 5.0f;
	...
	if(UnitToFollow != NULL)
	{
		...
		__try
		{
			if( UnitToFollow->event_GetInstanceID() != m_Unit->event_GetInstanceID() || !UnitToFollow->IsInWorld() )
				UnitToFollow = NULL;
			else
			{
#endif
...
/*
	...
	if( smallest_cool_time != MAX_INT && def_spell == NULL )
		...
		smallest_cool_time = MOB_SPELLCAST_REFRESH_COOLDOWN_INTERVAL;
	next_spell_time = nowtime + smallest_cool_time;
...
uint32 AIInterface::_CalcThreat(uint32 damage, SpellEntry * sp, Unit* Attacker)
{
	...
	if( sp != NULL && Need_SM_Apply(sp) && Attacker)
	{
...
bool AIInterface::CheckCurrentTarget()
{
	...
	{
		if( m_Unit->GetTypeId() == TYPEID_UNIT )
			cansee = SafeCreatureCast( m_Unit )->CanSee( GetNextTarget() );
		else
			cansee = SafePlayerCast( m_Unit )->CanSee( GetNextTarget() );
	}
	else 
	{
		WipeCurrentTarget();
