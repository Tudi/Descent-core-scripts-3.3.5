...
MapMgr::MapMgr(Map *map, uint32 mapId, uint32 instanceid) : CellHandler<MapCell>(map), _mapId(mapId), eventHolder(instanceid)
{
	...
	m_CreatureArraySize = map->CreatureSpawnCount + 1;
	m_GOArraySize = map->GameObjectSpawnCount + 1;
	...
	//if( mapId == 0 || mapId == 1 )
	{
		//common states for a priest and DK
		//useless craps ?
/*		SetWorldState(4354,1250262700);
		SetWorldState(4131,60);
		SetWorldState(3491,8);
		SetWorldState(3490,3);
		SetWorldState(2565,142);
		SetWorldState(4025,47);
		SetWorldState(4024,596);
		SetWorldState(4023,533);
		SetWorldState(4022,533);
		SetWorldState(4021,1066);
		SetWorldState(4020,33);	*/

		SetWorldState(3191,4);		//arena season 6 enabler ?
//		SetWorldState(3191,6);		//arena season 7 enabler ?
/*		SetWorldState(4375,0);
		SetWorldState(4273,0);
		...
		SetWorldState(4062,0); */
//		SetWorldState(0x00000F3D, 0x00000004);	//no idea about this one :(

		//3.3 lvl 80 mage with arenateam
		SetWorldState(4131,60);
		SetWorldState(4864,100);
		SetWorldState(4863,300);
		SetWorldState(4862,1000);
		SetWorldState(4354,1260471078);
		SetWorldState(3191,7);
		SetWorldState(2565,142);
		SetWorldState(4025,7);
		SetWorldState(4024,2181);
		SetWorldState(4023,300);
		SetWorldState(4022,300);
		SetWorldState(4584,0);
		SetWorldState(4582,0);
		SetWorldState(4440,0);
		SetWorldState(4439,0);
		SetWorldState(4273,0);
		SetWorldState(4857,0);
		SetWorldState(4375,0);
		SetWorldState(2324,0);
		SetWorldState(2260,0);
		SetWorldState(4062,0);
		SetWorldState(3083,0);
		SetWorldState(2325,0);
		SetWorldState(2322,0); 
	}
...
void MapMgr::SendInitialWorldState(Player *plr)
{
...
void MapMgr::PushObject(Object *obj)
{
	...
	if(plObj)
	{
		...
		plObj->PushCreationData(&temp_buff, count);
...
//		case HIGHGUID_TYPE_VEHICLE:
			{
				...
				m_CreatureStorage[obj->GetUIdFromGUID()] = SafeCreatureCast(obj);
		...
		case HIGHGUID_TYPE_GAMEOBJECT:
			{
				m_GOStorage[obj->GetUIdFromGUID()] = SafeGOCast(obj);
		...
		case HIGHGUID_TYPE_DYNAMICOBJECT:
			m_DynamicObjectStorage[obj->GetLowGUID()] = SafeDynOCast(obj);
			break;
		}
	}
...
//		case HIGHGUID_TYPE_VEHICLE:
	...
	for (Object::InRangeSet::iterator iter = obj->GetInRangeSetBegin();	iter != obj->GetInRangeSetEnd(); ++iter)
	{
		if( (*iter) )
		{
			...
			{
				if( SafePlayerCast( *iter )->IsVisible( obj ) && SafePlayerCast( *iter )->m_TransporterGUID != obj->GetGUID() )
	...
	if(!_shutdown && obj->GetTypeId() == TYPEID_PLAYER)
	{
		...
		m_PlayerStorage.erase( SafePlayerCast( obj )->GetLowGUID() );
	}
			...
			if( !obj->IsInRangeSet( curObj ) )
			{
				...
				if( curObj->IsPlayer() )
				{
					...
					if( plObj2->CanSee( obj ) && !plObj2->IsVisible( obj ) )
					{
						...
						plObj2->PushCreationData(&temp_buff, count);
...
//							SafeUnitCast( curObj )->SendAurasToPlayer( plObj );
			...
			//we are in visibility box but not in visibility square
			else
			{
				// Check visiblility
				if( curObj->IsPlayer() )
	...
	for(; iter != _updates.end();)
	{
		...
		else
		{
			if( pObj->IsInWorld() )
			{
				...
				// build the update
				if( pObj->GetInRangePlayersCount() )
				{
					temp_buff.clear();
				...
				if (cell)
				{
					...
					for(; iter != iend; ++iter)
					{
						...
						if(!plr->IsVisible(obj) && plr->CanSee(obj) && farsight->GetDistance2dSq(obj) <= m_UpdateDistance)
						{
							...
							plr->PushCreationData(&temp_buff, count);
...
/*	for(set<Object*>::iterator itr = _mapWideStaticObjects.begin(); itr != _mapWideStaticObjects.end(); ++itr)
	...
	// Sessions are updated every loop.
	{
		...
		SessionSet new_list( Sessions );	//updatesession changes the list. Mutex would deadlock
...
/*
Vehicle * MapMgr::CreateVehicle(uint32 entry)		//special creature
{
	uint64 newguid = ( (uint64)HIGHGUID_TYPE_VEHICLE << 32 );
...
//	uint32 pet_number = (sWorld.Global_Pet_id++) & 0xFF;	//not sure this is even required
//	newguid |= ( pet_number << 24 );
//	newguid |= ( (uint64)0x8100000000 );
...
//	uint64 new_guid = ( (uint64)HIGHGUID_TYPE_GAMEOBJECT << 32 ) | ( (uint64)entry << 24 ) | m_GOHighGuid;
	uint64 new_guid = ( (uint64)HIGHGUID_TYPE_GAMEOBJECT << 32 ) | m_GOHighGuid;
	return new GameObject( new_guid );
}
...
{ 
	...
	{ 
		return SVMap_z + 1.5f;	//if you manage to guess the value with less then 1 yard precission then wow will do the collision test and make a smooth movement. Else "fall" animation will be triggered
	}
...
uint32 GetProbableZoneIdForLocation(uint32 map_id, float x, float y)
{
