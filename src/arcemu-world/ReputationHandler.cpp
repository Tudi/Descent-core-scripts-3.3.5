...
void Player::SetStanding( uint32 Faction, int32 Value, int32 inloop )
{
	...
	{
		Event_Achiement_Received(ACHIEVEMENT_CRITERIA_TYPE_GAIN_REPUTATION,Faction,ACHIEVEMENT_UNUSED_FIELD_VALUE,newValue,ACHIEVEMENT_EVENT_ACTION_SET);
		Event_Achiement_Received(ACHIEVEMENT_CRITERIA_TYPE_KNOWN_FACTIONS,ACHIEVEMENT_UNUSED_FIELD_VALUE,ACHIEVEMENT_UNUSED_FIELD_VALUE,(uint32)m_reputation.size(),ACHIEVEMENT_EVENT_ACTION_SET_MAX);
		if( newValue >= STANDING_EXALTED )
		{
			ReputationMap::iterator itr1;
			uint32 exaltedcount=0;
			for( itr1 = m_reputation.begin();itr1 != m_reputation.end();itr1++)
				if( itr1->second->standing >= STANDING_EXALTED )
					exaltedcount++;
			Event_Achiement_Received(ACHIEVEMENT_CRITERIA_TYPE_GAIN_EXALTED_REPUTATION,ACHIEVEMENT_UNUSED_FIELD_VALUE,ACHIEVEMENT_UNUSED_FIELD_VALUE,exaltedcount,ACHIEVEMENT_EVENT_ACTION_SET_MAX);
		}
		if( newValue <= STANDING_REVERED )
		{
			ReputationMap::iterator itr1;
			uint32 reveredcount=0;
			for( itr1 = m_reputation.begin();itr1 != m_reputation.end();itr1++)
				if( itr1->second->standing <= STANDING_REVERED )
					reveredcount++;
			Event_Achiement_Received(ACHIEVEMENT_CRITERIA_TYPE_GAIN_REVERED_REPUTATION,ACHIEVEMENT_UNUSED_FIELD_VALUE,ACHIEVEMENT_UNUSED_FIELD_VALUE,reveredcount,ACHIEVEMENT_EVENT_ACTION_SET_MAX);
		}
	}
