...
public:
	...
	uint32	instance_next_save_stamp;	//avoid spamming sql with every npc kill

	void LoadFromDB(Field * fields);
	void SaveToDB();
	void DeleteFromDB();
	ARCEMU_INLINE uint32 GetCurrentPlayerLimit()
	{
		if( instance_difficulty == INSTANCE_MODE_DUNGEON_NORMAL || instance_difficulty == INSTANCE_MODE_DUNGEON_HEROIC )
			return 5;
		else if( instance_difficulty == INSTANCE_MODE_RAID_10_MAN || instance_difficulty == INSTANCE_MODE_RAID_10_MAN_HEROIC )
			return 10;
		else if( instance_difficulty == INSTANCE_MODE_RAID_25_MAN || instance_difficulty == INSTANCE_MODE_RAID_25_MAN_HEROIC )
			return 25;
		else 
			return 0x00FFFFFF;	//no limit
	}
};
...
public:
    ...
    ARCEMU_INLINE bool PlayerOwnsInstance(Instance * pInstance, Player * pPlayer)
	{
		...
		{
			return (pPlayer->GetPersistentInstanceId(pInstance->m_mapId, pInstance->instance_difficulty) == pInstance->m_instanceId);
		}
	...
	//might deadlock or crash server ? :O
	float GetAVGServerLatency()
	{
...
/*		m_mapLock.Acquire();
		...
		for(int i=0; i<NUM_MAPS; i++)
		{
			...
			if( instancemap )
				...
				{
					float cur_latency = ((float)itr->second->m_mapMgr->timer_exec_time / (float)itr->second->m_mapMgr->timer_exec_count);
					if( cur_latency > 0.1f )
					{
...
//		m_mapLock.Release();
		return server_latency / server_latency_counter;
	}

	InstanceMap **GetInstanceList() { return m_instances; }
	MapMgr **GetSingleMapList() { return m_singleMaps; }
	Map **GetMapList() { return m_maps; }
private:
	void _LoadInstances();
	...
	bool _DeleteInstance(Instance * in, bool ForcePlayersOut, bool skip_db_delete = false);

	uint32 m_InstanceHigh;

	Mutex m_mapLock;
	Map * m_maps[NUM_MAPS+1];
	InstanceMap* m_instances[NUM_MAPS+1];
	MapMgr * m_singleMaps[NUM_MAPS+1];
	time_t m_nextInstanceReset[NUM_MAPS+1];
};
