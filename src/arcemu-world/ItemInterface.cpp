...
void ItemInterface::EventItemAdd(Item *pItem)
{
	if( pItem == NULL )
		return;
//	CurrencyTypesEntry *ce = dbcCurrencyTypesStore.LookupEntryForced( pItem->GetUInt32Value(OBJECT_FIELD_ENTRY) ); 
	uint32 stack_to_add = pItem->GetUInt32Value(ITEM_FIELD_STACK_COUNT) ? pItem->GetUInt32Value(ITEM_FIELD_STACK_COUNT) : 1;
//	if( ce )
//		m_pOwner->ModUnsigned32Value( PLAYER_FIELD_CURRENCYTOKEN_SLOT_1 + ce->BitIndex, stack_to_add );

	m_pOwner->Event_Achiement_Received(ACHIEVEMENT_CRITERIA_TYPE_OWN_ITEM,pItem->GetUInt32Value(OBJECT_FIELD_ENTRY),ACHIEVEMENT_UNUSED_FIELD_VALUE, stack_to_add, ACHIEVEMENT_EVENT_ACTION_ADD);
	if( pItem->GetProto()->Quality >= ITEM_QUALITY_EPIC_PURPLE )
		m_pOwner->Event_Achiement_Received(ACHIEVEMENT_CRITERIA_TYPE_RECEIVE_EPIC_ITEM,ACHIEVEMENT_UNUSED_FIELD_VALUE,ACHIEVEMENT_UNUSED_FIELD_VALUE, stack_to_add, ACHIEVEMENT_EVENT_ACTION_ADD);
}
...
void ItemInterface::EventItemRemove(Item *pItem)
{
	if( pItem == NULL )
		return;
/*	CurrencyTypesEntry *ce = dbcCurrencyTypesStore.LookupEntryForced( pItem->GetUInt32Value(OBJECT_FIELD_ENTRY) ); 
	int32 stack_to_add = pItem->GetUInt32Value(ITEM_FIELD_STACK_COUNT) ? pItem->GetUInt32Value(ITEM_FIELD_STACK_COUNT) : 1;
	if( ce )
	{
		if( m_pOwner->ModUnsigned32Value( PLAYER_FIELD_CURRENCYTOKEN_SLOT_1 + ce->BitIndex ) >= stack_to_add )
			m_pOwner->ModUnsigned32Value( PLAYER_FIELD_CURRENCYTOKEN_SLOT_1 + ce->BitIndex, -stack_to_add );
		else
			m_pOwner->SetUnsigned32Value( PLAYER_FIELD_CURRENCYTOKEN_SLOT_1 + ce->BitIndex, 0 );
	}*/
}
...
Item *ItemInterface::SafeRemoveAndRetreiveItemByGuid(uint64 guid, bool destroy)
{
	...
	for(i = INVENTORY_CURRENCYTOKEN_SLOT_START; i < INVENTORY_CURRENCYTOKEN_SLOT_END; i++)
	{
		Item *item = GetInventoryItem(i);
...
#ifndef CATACLYSM_SUPPORT
	for(uint16 i=INVENTORY_KEYRING_START; i<INVENTORY_CURRENCYTOKEN_SLOT_END; i++)
	{
		if(m_pItems[i])
		{
			if(m_pItems[i]->GetProto()->ItemId == ID)
...
#ifndef CATACLYSM_SUPPORT
	for(uint16 i=INVENTORY_CURRENCYTOKEN_SLOT_START; i<INVENTORY_CURRENCYTOKEN_SLOT_END; i++)
	{
		if(m_pItems[i])
		{
			if(m_pItems[i]->GetGUID() == guid)
...
#ifndef CATACLYSM_SUPPORT
		else if( (*item)->GetProto()->BagFamily & ITEM_TYPE_CURRENCY || (*item)->GetProto()->Class == ITEM_CLASS_MONEY )
		{
			for(i=INVENTORY_CURRENCYTOKEN_SLOT_START; i<INVENTORY_CURRENCYTOKEN_SLOT_END; i++ )
			{
				if(m_pItems[i] == NULL)
				...
				{
					uint32 itemMaxStack = m_pItems[i]->GetProto()->MaxCount;
					if( m_pItems[i]->GetProto()->ItemId == (*item)->GetProto()->ItemId && itemMaxStack > 1 &&
...
#ifndef CATACLYSM_SUPPORT
			else if( proto->BagFamily & ITEM_TYPE_CURRENCY ||  proto->Class == ITEM_CLASS_MONEY )
			{
				for(uint32 i = INVENTORY_CURRENCYTOKEN_SLOT_START; i < INVENTORY_CURRENCYTOKEN_SLOT_END; i++)
				{
					if(m_pItems[i] == NULL)
					{
						count++;
...
uint8 ItemInterface::CanAffordItem(ItemPrototype * item,uint32 amount, Creature * pVendor)
{
	...
	if(item->BuyPrice)
	{
		...
		if((int32)m_pOwner->GetGold() < price)
		{
			return CAN_AFFORD_ITEM_ERROR_NOT_ENOUGH_MONEY;
		}
	}
...
{
	bool IgnoreSameEntry = ( newequip->Unique == 0 && (newequip->Flags & ITEM_FLAG_UNIQUE_EQUIP)==0 );
	for( uint32 x = EQUIPMENT_SLOT_START; x < EQUIPMENT_SLOT_END; ++x )
	{
		Item * it = m_pItems[x];
...
//	uint32 maxStack = chr->ItemStackCheat ? 0x7fffffff : it->MaxCount;
	...
	while( count > 0 && freeslots )
	{
		...
		if( AddItemToFreeSlot( &item ) )
		{
			...
			chr->Event_Achiement_Received(ACHIEVEMENT_CRITERIA_TYPE_OWN_ITEM,item->GetUInt32Value(OBJECT_FIELD_ENTRY),ACHIEVEMENT_UNUSED_FIELD_VALUE, toadd, ACHIEVEMENT_EVENT_ACTION_ADD);
			count -= toadd;
		}
		else
		{
