...
bool ChatHandler::HandleLearnSkillineCommand(const char* args, WorldSession *m_session)
{
	...
	sGMLog.writefromsession(m_session, "taught %s thought skilline %u", plr->GetName(),spell);
	plr->LearnAllSpellsFromSkillLine( spell, true, false );
	BlueSystemMessageToPlr(plr, "%s taught you skilline %d", m_session->GetPlayer()->GetName(), spell);
	return true;
}
...
bool ChatHandler::HandleAccountBannedCommand(const char * args, WorldSession * m_session)
{
	...
	*pDuration = 0;
	...
	if( m_session->HasGMPermissions() == false )
	{
		if( banned > FORCED_GM_BAN_LIMITATION )
			banned = FORCED_GM_BAN_LIMITATION;
...
bool ChatHandler::HandleParalyzeCommand(const char* args, WorldSession *m_session)
{
	...
	BlueSystemMessageToPlr( SafePlayerCast( plr ), "You have been rooted by %s.", m_session->GetPlayer()->GetName() );
	sGMLog.writefromsession( m_session, "rooted player %s", SafePlayerCast( plr )->GetName() );
	WorldPacket data;
...
bool ChatHandler::HandlePowerCheatCommand(const char* args, WorldSession* m_session)
{
	...
	for(int i= 0;i<7;i++)
		plyr->SetUInt32Value( UNIT_FIELD_POWER1 + i, plyr->GetUInt32Value( UNIT_FIELD_MAXPOWER1 + i ) );
	if(strcmp(args, "on") == 0)
	{
...
bool ChatHandler::HandleModifyLevelCommand(const char* args, WorldSession* m_session)
{
	...
	plr->Event_Achiement_Received(ACHIEVEMENT_CRITERIA_TYPE_REACH_LEVEL,ACHIEVEMENT_UNUSED_FIELD_VALUE,ACHIEVEMENT_UNUSED_FIELD_VALUE,Level,ACHIEVEMENT_EVENT_ACTION_SET_MAX);
	plr->Reset_Talents();
	return true;
}
...
bool ChatHandler::HandleCastAllCommand(const char* args, WorldSession* m_session)
{
	...
	for (itr = objmgr._players.begin(); itr != objmgr._players.end(); itr++)
	{
		...
		if(plr->GetSession() && plr->IsInWorld())
		{
			...
			{
				sEventMgr.AddEvent( SafeUnitCast( plr ), &Unit::EventCastSpell, SafeUnitCast( plr ), info, EVENT_PLAYER_CHECKFORCHEATS, 100, 1,EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT );
			}
			else
			{
...
{
#ifdef FORCED_GM_TRAINEE_MODE
	if( m_session->CanUseCommand('k') && !m_session->HasGMPermissions() && m_session->GetPlayer()->GetMapMgr() && m_session->GetPlayer()->GetMapMgr()->GetMapId() != 169 )
	{
