...
//	sEventMgr.AddEvent(this, &Creature::DeleteMe, EVENT_CREATURE_SAFE_DELETE, 1000, 1, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT );
	sEventMgr.AddEvent(this, &Creature::DeleteMe, EVENT_CREATURE_SAFE_DELETE, 1000, 1, EVENT_FLAG_DELETES_OBJECT );
}
...
//	lootmgr.FillCreatureLoot(&loot,GetEntry(), m_mapMgr ? (m_mapMgr->instance_difficulty) : 0);
	if( DificultyProto && DificultyProto->loot_kill )
...
void Creature::EnslaveExpire()
{
	...
	if(caster)
	{
		...
		sStackWolrdPacket( data, SMSG_PET_SPELLS, 50 );
		data << uint64(0);
...
//			pos += int32( 0.30f * owner->GetUInt32Value( UNIT_FIELD_STAT2 ) );	//says tooltip
			pos += int32( 0.75f * owner->GetUInt32Value( UNIT_FIELD_STAT2 ) );	//says wowwiki
...
//	heroic_entry3 - 25 heroic
	...
	SetUInt32Value(UNIT_FIELD_POWER1,DificultyProto->Mana);
	SetUInt32Value(UNIT_FIELD_MAXPOWER1,DificultyProto->Mana);
	...
	else SetUInt32Value(UNIT_FIELD_DISPLAYID,model);
#endif
	...
	if( DificultyProto != proto && mode && info)
	{
		...
		ModUnsigned32Value(UNIT_FIELD_LEVEL, min( sWorld.m_levelCap + 3 - GetUInt32Value(UNIT_FIELD_LEVEL), exrta_level));
	}

	uint32 health = DificultyProto->MinHealth + RandomUInt(DificultyProto->MaxHealth - DificultyProto->MinHealth);
	if( DificultyProto != proto && IS_HEROIC_INSTANCE_DIFFICULTIE( mode ) )
...
//		m_aiInterface->addSpellToList(*itr);
	...
	m_aiInterface->m_moveFly = ( m_aiInterface->m_moveFly == true) | (( DificultyProto->extra_a9_flags & 0x400 ) != 0);
...
/*		uint32 newhealth = m_uint32Values[UNIT_FIELD_HEALTH] / 100;
		...
		setDeathState( ALIVE );
	m_invisFlag = DificultyProto->invisibility_type;
...
//	SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_DISPLAY_02, proto->Item3SlotID);
	...
	m_aiInterface->m_moveFly = ( m_aiInterface->m_moveFly == true) | (( proto->extra_a9_flags & 0x400 ) != 0);
...
/*		if(m_spawn->channel_target_creature)
	...
	if( m_mapMgr && IS_MAP_INSTANCE( m_mapMgr->GetMapInfo() ) )
		m_aiInterface->m_is_in_instance = true;
	else 
