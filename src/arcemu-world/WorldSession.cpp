...
void WorldSession::LogoutPlayer(bool Save)
{
	...
	if( _player != NULL )
	{
		...
		if( plr->IsInGuild() )
		{
			...
			if( pGuild != NULL )
				pGuild->LogGuildEvent( GUILD_EVENT_HASGONEOFFLINE, 1, plr->GetName() );
		}
		...
		plr->load_mana = plr->GetUInt32Value( UNIT_FIELD_POWER1 );

		if( plr->GetSummon() != NULL )
		{
			plr->GetSummon()->Remove( true, true );
			plr->SetSummon( NULL );
		}
...
//	WorldPacketHandlers[CMSG_CLEAR_LOOKING_FOR_GROUP].handler					= &WorldSession::HandleMultipleActivateTaxiOpcode;
	...
	WorldPacketHandlers[MSG_GUILD_BANK_MONEY_WITHDRAWN].handler								= &WorldSession::HandleGuildLog;
...
//	WorldPacketHandlers[CMSG_CHANNEL_VOICE_QUERY].handler = &WorldSession::HandleChannelVoiceQueryOpcode;
	...
	WorldPacketHandlers[CMSG_CHANGE_SEATS_ON_CONTROLLED_VEHICLE].handler =		&WorldSession::HandleVehicleSwitchSeat;
...
#define local_buffer_size 15000 //this is due to stack limitation for profiler :S
	...
	if( session->GetPlayer() )
		CharacterDatabase.Execute( "INSERT INTO gm_logs values( '','%u','%u','%s' )", (unsigned int)session->GetAccountId(), session->GetPlayer()->GetLowGUID(), CharacterDatabase.EscapeString( out ).c_str() );
	else
		CharacterDatabase.Execute( "INSERT INTO gm_logs values( '','%u','0','%s' )", (unsigned int)session->GetAccountId(), CharacterDatabase.EscapeString( out ).c_str() );
