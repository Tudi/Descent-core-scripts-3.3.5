...
enum SPELL_MODIFIER_TYPE
{
    ...
    SMT_CAST_COOLDOWN       =21,// 302->reduces global cooldown
    ...
    SMT_ADD_TO_EFFECT_VALUE_2      =41,//only added when spell value 1 is calculated
    SMT_ADD_TO_EFFECT_VALUE_3      =42,//only added when spell value 1 is calculated
};

#define SPELL_RADIUS_INDEX_2YARDS	7	//for melee stuff

#define USE_SPELL_DOWNRANKING			//used by blizz so spells that have a few ranks to be weaker then the ones having lots of ranks

#define SPELL_POWER_TO_HEALING_POWER_CONVERSION_FACTOR	1.3f		//some say that arcemu heals are imba. Maybe reduce this ?

#define DEFAULT_SPELL_PROC_INTERVAL	1000		//avoid proc exploits ( some people managed to proc over proc spells creating insane DPS, this protection is only for future bug avoiding)
...
enum VisualAuraSendFlags
{
	...
	VISUALAURA_FLAG_NEGATIVE		= 0x80, //red icon clietn side
};
...
enum procFlags
{
	...
	PROC_ON_CAST_SPELL					= 0x10,		//POST CAST EVENT !!
	...
	PROC_ON_ANY_SPELL_DAMAGE_DONE		= 0x10000,
	...
	PROC_ON_HEAL_SPELL					= 0x8000000,	//remove prochcharge only when it is used
	PROC_SCRIPTED_PROCTEST				= 0x10000000,	//We will always script the proc condition for these flags
	PROC_ON_BLOCK_VICTIM				= 0x20000000,
	PROC_ON_SPELL_CRIT_HIT				= 0x40000000,
	PROC_FREEE							= 0x80000000,//our custom flag to decide if proc target is self or victim
};
...
enum AuraInterruptFlags
{
    ...
    AURA_INTERRUPT_ON_ANY_DAMAGE_TAKEN_35_PCT_CHANCE    = 0x200,
...
enum SpellEnchantmentTypes
{
	...
	SPELL_ENCHANT_TYPE_MOD_RATING						= 5,
...
enum SpellIsFlags
{
	...
	SPELL_FLAG_IS_DISABLE_OTHER_SPELL_CPROC	= 0x80000000, // almost not profession spell procs other spells
};
...
enum DiminishingGroup
{
	...
	DIMINISHING_GROUP_MISS_DODGE_PARRY_BLOCK				= 16,	//all together because we are using stacking chances
...
ARCEMU_INLINE bool IsDamagingSpell(SpellEntry *sp, uint32 ind)
{
    ...
    if( sp->Effect[ind]==SPELL_EFFECT_APPLY_AURA || sp->Effect[ind]==SPELL_EFFECT_APPLY_AREA_AURA || sp->Effect[ind]==SPELL_EFFECT_APPLY_AREA_AURA2 )
    {
...
//        case SPELL_EFFECT_HEALTH_FUNNEL:
    ...
    if( sp->Effect[i] == SPELL_EFFECT_APPLY_AURA ||
		...
		sp->Effect[i] == SPELL_EFFECT_APPLY_AREA_AURA2 )
    {
...
typedef enum {
   ...
   EFF_TARGET_LIST_LENGTH_MARKER						= 106,
} SpellEffectTarget;
...
//    void SpellEffectCreateSummonTotem(uint32 i);
    ...
    void SpellEffectRuneToPower(uint32 i);
...
/*        switch(GetProto()->NameHash)
	...
	signed int	forced_miscvalues[3];	//some proc spells use settings based on previous actions : resist last received spell type
	signed int	static_dmg[3];			//the forced values represent full values and not extra value should be added to them
	signed int	forced_duration; //some proc spells can override default duration
...
public:
    ...
    TargetsList m_targetUnits[3];		//watch it though !
...
{ 
	if(!m_spellProto) 
		return false; 		
	return (((m_spellProto->Attributes & ATTRIBUTES_PASSIVE) || (m_spellProto->AttributesEx & ATTRIBUTESEX_VISIBLE)) && ( m_spellProto->spellIconID != 1 || m_spellProto->activeIconID != 0 ));
}
