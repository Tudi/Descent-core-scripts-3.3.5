...
{
public:
	CreatureProto() { on_click_spell = 0; vehicleId = 0; vehicle_use_required_quest = 0;}
	uint32 Id;
	uint32 MinLevel;
	uint32 MaxLevel;
	uint32 Faction;
...
class AuctionHouse;
struct Trainer;
#define CALL_SCRIPT_EVENT(obj, func) if(obj->GetTypeId() == TYPEID_UNIT && SafeCreatureCast(obj)->GetScript() != NULL) SafeCreatureCast(obj)->GetScript()->func
...
public:
	...
	ARCEMU_INLINE uint32 GetItemAmountBySlot(uint32 slot) 
	{
		INSTRUMENT_TYPECAST_CHECK_CREATURE_OBJECT_TYPE
		return m_SellItems->at(slot).amount; 
	}
	...
	tr1::array<float,7> ModDamageDonePct;
