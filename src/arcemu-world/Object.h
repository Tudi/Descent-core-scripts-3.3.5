...
enum HIGHGUID_TYPE
{
	HIGHGUID_TYPE_ITEM				= 0x40000000, // do not seem to have entry in guid
	HIGHGUID_TYPE_CONTAINER			= 0x40000000, // it is same as item in 3.1.1 and probably should be the same in the future also cause we are not saving highguid in db		
	HIGHGUID_TYPE_UNIT				= 0xF1300000,
	HIGHGUID_TYPE_PET				= 0xF1400000, // found in 3.1.3 TBC
//	HIGHGUID_TYPE_VEHICLE			= 0xF1500000,
	HIGHGUID_TYPE_GAMEOBJECT		= 0xF1C00000, // also found F1 10 02 B6
...
//	ARCEMU_INLINE std::set<Object*>::iterator GetInRangeOppFactsSetEnd() { return m_oppFactsInRange.end(); }
	...
	//very higly populated realms to reduce lag
	//the idea is to send spell dmglogs or melee dmg logs to duel players only
	ARCEMU_INLINE void SendMessageToDuel(StackWorldPacket * data);
	...
	void SpellNonMeleeDamageLog(Unit *pVictim, SpellEntry *spellInfo, uint32 damage, bool allowProc, bool static_damage = false, bool no_remove_auras = false,uint32 DOTticks=1,int32 pct_mod=100);
	void SpellNonMeleeDamageLog(Unit *pVictim, uint32 SpellId, uint32 damage, bool allowProc, bool static_damage = false, bool no_remove_auras = false,uint32 DOTticks=1);
...
//	ARCEMU_INLINE size_t GetInRangeOppFactCount() { return m_oppFactsInRange.size(); }
	...
	void PlaySoundTarget(uint32 sound_entry, uint64 &guid);
	bool IsInBg();
	uint32 GetTeam();
...
public:
	...
	{
		m_extensions.insert( make_pair( ID, ptr ) );
	}
