...
void Vehicle::LoadFromDB()
{
	...
	{
		printf("No or invalid vehicle data was found for vehicleid %u, removing vehicle from world.", VehicleID);
		return;
	}
...
{
	sStackWolrdPacket( data, SMSG_MONSTER_MOVE_TRANSPORT, 100 + 100);
	data << passenger->GetNewGUID();								// Passengerguid
...
{
	sStackWolrdPacket( data, SMSG_PET_SPELLS, 63+5);
	data << GetGUID();
