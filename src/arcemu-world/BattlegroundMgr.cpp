		...
		for(j = 0; j < MAX_LEVEL_GROUP; ++j)
		{
			...
			for(iitr = m_instances[i].begin(); iitr != m_instances[i].end(); ++iitr)
			{
				...
				else
				{
					...
					while (tempPlayerVec[0].size() > 0 && bg->GetBalanceFreeSlots(0, bg->GetType()))
						AddPlayerToBgTeam(bg, &tempPlayerVec[0], i, j, 0);
					while (tempPlayerVec[1].size() > 0 && bg->GetBalanceFreeSlots(1, bg->GetType()))
...
void CBattlegroundManager::NotifyPlayersAboutStatus(Player *plr_change)
{
	...
	sprintf( msg_to_send,"Battleground : Player %s changed the queue. Hordes in queue %u and Aliance in queue %u.Need %u to start",plr_change->GetName(),sides[1],sides[0],minPlayers);
	itr = m_queuedPlayers[plr_change->m_bgQueueType][lgroup].begin();
	while(itr != m_queuedPlayers[plr_change->m_bgQueueType][lgroup].end())
	{
		Player *tplr = objmgr.GetPlayer(*itr);
...
{
	if( IS_ARENA( dbcIndex ) )
		return ((dbcIndex - BATTLEGROUND_ARENA_2V2) + 2)*2;	//arenas in DBC require 5 players instead of 2,3,4,5
	//in 3.3.2 min player is not found in dbc anymore
	if( dbcIndex == BGDBC_WARSONG_GULCH )
		return 5;
...
void CBattleground::BuildPvPUpdateDataPacket(WorldPacket * data)
{
	...
	if(m_type >= BATTLEGROUND_ARENA_2V2 && m_type <= BATTLEGROUND_ARENA_5V5)
	{
		...
		*data << uint8(1);
		...
		{
			if(static_cast<Arena*>(this)->GetTeam(0)) {
				*data << uint32(0) << uint32(3000+m_deltaRating[0]) << uint8(0);
			} else {
				*data << uint32(0) << uint32(0) << uint8(0);
			}

			if(static_cast<Arena*>(this)->GetTeam(1)) {
...
void CBattleground::OnPlayerPushed(Player * plr)
{
	...
	{
		if ( plr->m_isGmInvisible == false && m_groups[plr->m_bgTeam] ) //do not join invisible gm's into bg groups.
...
void CBattleground::PortPlayer(Player * plr, bool skip_teleport /* = false*/)
{
	...
	if(!skip_teleport)
	{
		...
		BattlegroundManager.SendBattlefieldStatus(plr, BF_STATUS_PACKET_TEL_PLAYER, m_type, m_id, (uint32)UNIXTIME - m_startTime, m_mapMgr->GetMapId(),Rated());   // Elapsed time is the last argument
	}
	else
	{
...
{
	m_mainLock.Acquire();
	set<Player*>::iterator itr,itr2;
	for(int i = 0; i < 2; ++i)
     for(set<Player*>::iterator itr = m_players[i].begin(); itr != m_players[i].end();)
	 {
		 itr2 = itr;
		 itr++;
...
00 
07 
02 00 00 00  - BATTLEGROUND_WARSONG_GULCH
90 1F 50 55 ?
...
02 00 00 00
90 1F 50 55 
05 00 00 00 instance id
00 uint8
02 00 00 00 status
E9 01 00 00 map
40 9C 00 00 time

00 00 00 00 00 07 02 00 00 00 90 1F 50 55 05 00 00 00 00 03 00 00 00 E9 01 00 00 00 00 00 00 B6 DE 07 00 00 8103828
...
05 00 00 00 - instance id ?
00 
03 00 00 00 - status = tel
E9 01 00 00 - map
00 00 00 00 - ?
B6 DE 07 00 00 - time ?

00 00 00 00 - queue slot
02 - arena 2 vs 2
0E 
06 00 00 00 
90 1F 
...
20 00 00 00
90 1F 50 50
00 00 00 00 instance id
00 
01 00 00 00 status = BF_STATUS_PACKET_QUEUE_PLAYER
16 74 00 00 - time
00 00 00 00 - elapsed time
...
*/
	...
	if(Status == BF_STATUS_PACKET_REMOVE_PLAYER)
		data << uint64(1) << uint32(0);
	else
	{
		...
		switch(Status)
		{
		case BF_STATUS_PACKET_QUEUE_PLAYER:             // Waiting in queue
			data << uint32(60) << uint32(0);			// Time / Elapsed time
			break;
		case BF_STATUS_PACKET_ALLOW_ENTER_PLAYER:       // Ready to join!
			data << uint32(0) << uint8(0) << uint8(0) << uint8(0) << Time;
			break;
		case BF_STATUS_PACKET_TEL_PLAYER:
			if(Type >= BATTLEGROUND_ARENA_2V2 && Type <= BATTLEGROUND_ARENA_5V5)
				data << MapId << uint32(120000) << Time << uint8( RatedMatch );	
			else
...
//	pCreature->SetUInt32Value(UNIT_FIELD_BYTES_0, 0 | (2 << 8) | (1 << 16));
	...
	pCreature->SetUInt32Value(UNIT_FIELD_FLAGS, UNIT_FLAG_PLUS_MOB | UNIT_FLAG_NOT_ATTACKABLE_9 | UNIT_FLAG_UNKNOWN_10); // 832
...
uint32 CBattleground::GetBalanceFreeSlots(uint32 Team, uint32 type)
{
	...
	if (type >= BATTLEGROUND_ARENA_2V2 && type <= BATTLEGROUND_ARENA_5V5)
	{
		res = maxPlayers - ((uint32)m_players[Team].size() + m_pendPlayers[Team].size());
	}
	else
	{
		uint32 size[2];
