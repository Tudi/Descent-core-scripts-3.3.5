					...
					if( sg1 != NULL)
					{
						for( itr1 = sg1->GetGroupMembersBegin(); itr1 != sg1->GetGroupMembersEnd(); ++itr1 )
						{
							...
							if( (*itr1)->m_loggedInPlayer == NULL )
								continue;
							(*itr1)->m_loggedInPlayer->Event_Achiement_Received(ACHIEVEMENT_CRITERIA_TYPE_COMPLETE_RAID,m_MemberCount,ACHIEVEMENT_UNUSED_FIELD_VALUE,1,ACHIEVEMENT_EVENT_ACTION_SET_MAX);
						}
					}
				}
			}
...
//				data << uint64(0x500000000004BC0CULL);
				data << uint32(0x0366B5E1);
				data << uint32(0x1F570000);
				static uint32 group_packet_serializer = 0;
				data << uint32( group_packet_serializer++ );		// 3.3 this value increments every time SMSG_GROUP_LIST is sent
...
{
	sStackWolrdPacket( data, SMSG_GROUP_DESTROYED, 5 );
	WorldPacket data2( SMSG_PARTY_COMMAND_RESULT, 20);
	data2 << uint32(2) << uint8(0) << uint32(m_Parent == NULL ? 0 : m_Parent->dungeon_difficulty)<< uint32(0);	// you leave the group
	data2 << uint32(m_Parent == NULL ? 0 : m_Parent->raid_difficulty);
...
void Group::RemovePlayer(PlayerInfo * info)
{
	...
	if( pPlayer != NULL )
	{
		if( pPlayer->GetSession() != NULL )
		{
			...
			data << uint32(2) << uint8(0) << uint32(0) << uint32(0);  // you leave the group
			pPlayer->GetSession()->SendPacket( &data );
		}
