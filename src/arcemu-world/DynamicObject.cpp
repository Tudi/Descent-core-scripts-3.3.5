...
void DynamicObject::Create(Unit * caster, Spell * pSpell, float x, float y, float z, uint32 duration, float radius,int32 spell_val)
{
...
//	m_floatValues[DYNAMICOBJECT_POS_Z]  = z;
	SetFloatValue( OBJECT_FIELD_SCALE_X, radius / 4.0f );	//no idea actually, but consacration spell had too small visual size 
	...
	sEventMgr.AddEvent(this, &DynamicObject::UpdateTargets, EVENT_DYNAMICOBJECT_UPDATE, 500, 0,EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
}
...
//						pAura->AddMod(m_spellProto->EffectApplyAuraName[i],	m_spellProto->EffectBasePoints[i]+1, m_spellProto->EffectMiscValue[i], i);
						pAura->AddMod(m_spellProto->EffectApplyAuraName[i],	spell_value, m_spellProto->EffectMiscValue[i], i);
					}
				}
				target->AddAura(pAura);
				if(p_caster)
					p_caster->HandleProc( PROC_ON_CAST_SPELL,target, m_spellProto);
