...
#define VMAP_SAVE_INTERVAL				4*60*60*1000	//for consistency map is dumped into file
...
#define VMAP_VALUE_BOTTOM_OF_THE_OCEAN	-5000			//no way to have values like this in maps
#define VMAP_VALUE_FAR_IN_THE_SKY		5000			//no way to have values like this in maps
#define VMAP_CUR_VERSION				16				//mark in header if we can load this saved file or not
#define VMAP_SUBCELL_SIZE				100				//maps are divided into 2 levels, high level and low level
#define VMAP_MAX_MAP_SIZE				(50000/VMAP_SCALE_WOW_MAP_RES)		//the amount of info we can hold
...
#define	VMAP_STABLE_STATISTIC_VALUE		0x8FFF			//just a high enough value to be able to say for sure that no errors were saved here
...
struct VMapCell
{
	...
	signed int		floor_sums[VMAP_FLOOR_COUNT];		//low floor and higy floor
	unsigned short	floor_sums_count[VMAP_FLOOR_COUNT];
};
