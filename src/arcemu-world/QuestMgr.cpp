...
void QuestMgr::BuildRequestItems(WorldPacket *data, Quest* qst, Object* qst_giver, uint32 status, uint32 language)
{
	...
	*data << uint32(0);				 // ?
	*data << uint32(0);				 // Emote for complete or incomplete ?

	*data << uint32(1);				 // close on cancel ?
	*data << uint32(qst->quest_flags);                     // 3.3.3 questFlags
	*data << uint32(qst->suggestedplayers);          // SuggestedGroupNum
	*data << uint32( qst->rew_money_at_max_level );	   // Required Money
...
//		plr->GiveTalent(qst->rewardtalents);
	}
...
*/
	...
	*data << qst_giver->GetGUID();
	*data << "How can I help you?"; //Hello line. Ex : the hex as text : Ah, well aren't you a sturdy-looking one? Perhaps you can assist me with a thing or two. Not much help around here except for green apprentices, and they've other things to worry about. 
...
void QuestMgr::OnQuestFinished(Player* plr, Quest* qst, Object *qst_giver, uint32 reward_slot)
{
    ...
    if(IsQuestRepeatable(qst) || IsQuestDaily(qst) || IsQuestWeekly(qst))
    {
		...
		plr->Event_Achiement_Received( ACHIEVEMENT_CRITERIA_TYPE_MONEY_FROM_QUEST_REWARD,ACHIEVEMENT_UNUSED_FIELD_VALUE,ACHIEVEMENT_UNUSED_FIELD_VALUE,rew_money,ACHIEVEMENT_EVENT_ACTION_ADD);
