...
uint32 InstanceMgr::PreTeleport(uint32 mapid, Player * plr, uint32 instanceid)
{
	...
	else
	{
		...
		else
		{
			...
			if(pGroup != NULL)
			{
				uint32 g_m_difficulty = pGroup->GetInstanceDifficulty();
				uint32 group_saved_instance_id = pGroup->GetSavedInstance( mapid, g_m_difficulty );
				if( IS_PERSISTENT_MAP( inf ) || IS_HEROIC_INSTANCE_DIFFICULTIE( g_m_difficulty ) )
				{
					if( plr->GetPersistentInstanceId(mapid, g_m_difficulty) == 0 )
					{
						if( group_saved_instance_id != GROUP_SAVED_INSTANCE_MISSING )
						{
						...
						else if(sWorld.instance_TakeGroupLeaderID)
						{
							...
							if(pLeaderInfo)
							{
								...
								PlayerInstanceMap::iterator itrLeader = pLeaderInfo->savedInstanceIds[g_m_difficulty].find(mapid);
								if(itrLeader != pLeaderInfo->savedInstanceIds[g_m_difficulty].end())
								{
					...
					if(in == NULL && plr->GetPersistentInstanceId(mapid, g_m_difficulty) != 0)
					{
						in = sInstanceMgr.GetInstanceByIds(mapid, plr->GetPersistentInstanceId(mapid, g_m_difficulty));
					}
				}
				else
				{
		...
		if(m_instances[i] != NULL)
		{
			...
			for(itr = instancemap->begin(); itr != instancemap->end();)
			{
				...
				if(in->m_persistent && PlayerOwnsInstance(in, plr))
				{
					...
					data << uint32( 0x1F410000 );					//seems to be a constant ?
