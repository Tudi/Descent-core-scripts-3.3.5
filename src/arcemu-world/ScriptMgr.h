...
enum ServerHookEvents
{
	...
	SERVER_HOOK_EVENT_ON_REMOVE_FROM_WORLD	= 30,
...
typedef void(*tOnRemoveFromWorld)(Unit * pUnit);
...
typedef CreatureAIScript*(*exp_create_creature_ai)(Creature * pCreature);	//called on push to world
...
protected:
	...
	ServerHookList _hooks[NUM_SERVER_HOOKS+1];
...
public:
	...
	virtual void StringFunctionCallStr(const char * pFunc) {}
	virtual void StringFunctionCall(int pFunc) {}
...
public:
	...
	virtual ~InstanceScript() {mInstance = NULL;};
...
public:
	...
	void OnRemoveFromWorld(Unit *pUnit);
