...
enum AURA_INTERNAL_USAGE_FLAGS
{
	...
	MOD_1_RESISTED							= 2,
	MOD_2_RESISTED							= 4,
	NEED_STATEIGNORE_RESEND_ON_MAPCHANGE	= 8,
};
...
enum AURA_STAE_FLAGS
{
    ...
    AURASTATE_FLAG_HEALTH_BELOW_20_PCT  = 2,        //2
...
//	AURASTATE_FLAG_HUNTER_CRIT			= 512,		//10
    ...
    AURASTATE_FLAG_HEALTH_BELOW_35_PCT  = 4096,     //13
    AURASTATE_FLAG_IMMOLATE             = 8192,     //14
    AURASTATE_FLAG_REJUVENATE           = 16384,    //15 //where do i use this ?
    AURASTATE_FLAG_POISON               = 32768,    //16
    AURASTATE_FLAG_ENRAGE               = 65536,    //17
    AURASTATE_FLAG_BLEED                = 131072,   //18
    AURASTATE_HYPOTHERMIA               = 262144,   //19
    AURASTATE_HEALTH_ABOVE_75_PCT		= 4194304,  //23
};
...
struct Modifier
{
    uint32	m_type;				// Aura type -> ex mod_hp
...
{
	ProcTriggerSpell(SpellEntry *sp) 
	{ 
		LastTrigger = 0; 
		deleted = false; 
		origId = sp->Id;
	...
	//    SpellEntry *spinfo;
	uint32 LastTrigger;
	int32 created_with_value;	//contain the amount the spell used when we casted it 
	void  *custom_holder;
...
public:
	...
	{ 
		if(!m_spellProto) 
			return false; 
		return ( (m_spellProto->Attributes & ATTRIBUTES_PASSIVE) && m_areaAura == false );
	}
...
//	void SpellAuraIncreaseHealingByInt(bool apply);
	...
	void SpellAuraPeriodicDummyTrigger(uint32 mod_i);
...
#define AURA_SpellAuraIncreaseAPBasedOnArmor_REFRESH_INTERVAL 2*60*1000
	...
	// Events
	void EventPeriodicDamage(uint32 amount,uint32 ticks);
	void EventPeriodicDamagePercent(uint32);
	void EventPeriodicHeal(uint32 amount,int32 period);
...
/*	ARCEMU_INLINE bool IsCombatStateAffecting()
	...
	tr1::array< Modifier, MAX_AURA_MOD_COUNT> m_modList;
private:
