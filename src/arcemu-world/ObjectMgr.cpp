...
void ObjectMgr::LoadVendors()
{
	...
	QueryResult *result = WorldDatabase.Query("SELECT a.entry,a.item,a.amount,a.max_amount,a.inctime,a.extended_cost FROM vendors a INNER JOIN  items b ON a.item = b.entry ORDER BY b.name1");
		...
		do
		{
			...
			if( sf_spellId )
			{
				...
				if( sp != NULL )
				{
					...
					{
						sp->SpellGroupType[0] = sf_SpellGroupType;
					...
					if ( sf_groupRelation0)
					{
						...
						sp->EffectSpellGroupRelation[0][1] = (uint32)(sf_groupRelation0);
					}
					if ( sf_groupRelation1)
					{
						sp->EffectItemType[1] = (uint32)sf_groupRelation1;
						sp->EffectSpellGroupRelation[1][1] = (uint32)(sf_groupRelation1);
					}
					if ( sf_groupRelation2)
					{
						sp->EffectItemType[2] = (uint32)sf_groupRelation2;
						sp->EffectSpellGroupRelation[2][1] = (uint32)(sf_groupRelation2);
					}
				}
			}
		}while(result->NextRow());
			...
			if( seo_SpellId )
			{
				...
				if( sp != NULL )
				{
					...
					if( seo_SpellGroupRelation )
						sp->EffectItemType[seo_EffectId] = seo_SpellGroupRelation;
...
uint32 ObjectMgr::GetPetSpellCooldown(uint32 SpellId)
{
	...
	return (sp->StartRecoveryTime + MAX(sp->CategoryRecoveryTime,sp->RecoveryTime));
}
...
//!!! only some spells have next level set. Maybe only pets ? 
...
/*	if( sp == NULL )
	...
	while( sp && sp->baseLevel <= level )
		sp = dbcSpell.LookupEntry( sp->spell_learn_next_level_spell );
...
void ObjectMgr::AddIPMute( const char *IP, uint32 duration_seconds )
{
	...
	CharacterDatabase.Query( "replace into IP_mutes (IP,expire_at) values( '%s',%d)",IP,expiresat );
}
