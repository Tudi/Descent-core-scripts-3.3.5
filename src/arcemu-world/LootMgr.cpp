...
void LootRoll::PlayerRolled(Player *player, uint8 choice)
{
	...
	*/
	...
	if(choice == LOOT_ROLL_TYPE_NEED) 
	{
		player->Event_Achiement_Received( ACHIEVEMENT_CRITERIA_TYPE_ROLL_NEED_ON_LOOT,roll,ACHIEVEMENT_UNUSED_FIELD_VALUE,1,ACHIEVEMENT_EVENT_ACTION_SET);
		player->Event_Achiement_Received( ACHIEVEMENT_CRITERIA_TYPE_ROLL_NEED,ACHIEVEMENT_UNUSED_FIELD_VALUE,ACHIEVEMENT_UNUSED_FIELD_VALUE,1,ACHIEVEMENT_EVENT_ACTION_ADD);
	...
	else if(choice == LOOT_ROLL_TYPE_GREED || choice == LOOT_ROLL_TYPE_DISENCHANT)
	{
		...
		{
			player->Event_Achiement_Received( ACHIEVEMENT_CRITERIA_TYPE_ROLL_GREED_ON_LOOT,roll,ACHIEVEMENT_UNUSED_FIELD_VALUE,1,ACHIEVEMENT_EVENT_ACTION_SET);
			player->Event_Achiement_Received( ACHIEVEMENT_CRITERIA_TYPE_ROLL_GREED,ACHIEVEMENT_UNUSED_FIELD_VALUE,ACHIEVEMENT_UNUSED_FIELD_VALUE,1,ACHIEVEMENT_EVENT_ACTION_ADD);
		}
		else
		{
			player->Event_Achiement_Received( ACHIEVEMENT_CRITERIA_TYPE_ROLL_DISENCHANT,ACHIEVEMENT_UNUSED_FIELD_VALUE,ACHIEVEMENT_UNUSED_FIELD_VALUE,1,ACHIEVEMENT_EVENT_ACTION_ADD);
		}
...
//		if( drop_chance == 0 )
	...
	{
		if( Rand( drop_chance * sWorld.getRate( RATE_DROP0 )) == true )
...
//		uint32 ti = (li * random_increment_index ) % max_ind;	//need to avoid parsing the same item twice
		...
		if( ls->ref_list )
		{
			...
			if( ls->DropChance<=0 || Rand( ls->DropChance * sWorld.getRate( RATE_DROP0 )) == true ) 
			{	
