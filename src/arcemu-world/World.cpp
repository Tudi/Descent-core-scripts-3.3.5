...
#endif
	Global_Pet_id = 1;		//nevah to have 0 !
}
...
void GenerateBoosetedAchievementLookupTable()
{
    for (uint32 i=0; i<dbcAchievementCriteriaStore.GetNumRows(); i++)
    {
		...
		dbcBoostedAchievementLookup[ criteria->requiredType ].push_back( criteria );
	}
	//no idea how to get required instance difficulty mode from DBC atm
	std::list<AchievementCriteriaEntry*>::iterator itr;
    for (itr = dbcBoostedAchievementLookup[ACHIEVEMENT_CRITERIA_TYPE_KILL_CREATURE].begin(); itr != dbcBoostedAchievementLookup[ACHIEVEMENT_CRITERIA_TYPE_KILL_CREATURE].end(); itr++)
    {
    ...
    for (itr = dbcBoostedAchievementLookup[ACHIEVEMENT_CRITERIA_TYPE_WIN_ARENA].begin(); itr != dbcBoostedAchievementLookup[ACHIEVEMENT_CRITERIA_TYPE_WIN_ARENA].end(); itr++)
    {
		uint32 achientry = (*itr)->ID;
		AchievementEntry *achi = dbcAchievementStore.LookupEntry( (*itr)->referredAchievement );
		if( !achi )
			continue;
		if( strstr( achi->name,"2vs2" ) )
			(*itr)->requires_arenta_team_type_check = ARENA_TEAM_TYPE_2V2;
		else if( strstr( achi->name,"3vs3" ) )
