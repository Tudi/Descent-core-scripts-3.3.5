...
void WorldSession::HandleActivateTaxiOpcode( WorldPacket & recv_data )
{
	...
	GetPlayer()->Event_Achiement_Received(ACHIEVEMENT_CRITERIA_TYPE_GOLD_SPENT_FOR_TRAVELLING,ACHIEVEMENT_UNUSED_FIELD_VALUE,ACHIEVEMENT_UNUSED_FIELD_VALUE,taxipath->GetPrice(),ACHIEVEMENT_EVENT_ACTION_ADD);
	GetPlayer()->Event_Achiement_Received(ACHIEVEMENT_CRITERIA_TYPE_FLIGHT_PATHS_TAKEN,ACHIEVEMENT_UNUSED_FIELD_VALUE,ACHIEVEMENT_UNUSED_FIELD_VALUE,1,ACHIEVEMENT_EVENT_ACTION_ADD);
	...
	if(_player->GetSummon() != NULL)
	{
		if(_player->GetSummon()->GetUInt32Value(UNIT_CREATED_BY_SPELL) > 0)
			_player->GetSummon()->Dismiss();						   // warlock summon -> dismiss
		else
			_player->GetSummon()->Remove(true, true);					  // hunter pet -> just remove for later re-call
