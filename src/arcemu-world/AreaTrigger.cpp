...
//		return AREA_TRIGGER_FAILURE_NO_HEROIC;
	...
	if( IS_MAP_RAID( pMapInfo ) && (!pPlayer->GetGroup() || (pPlayer->GetGroup() && pPlayer->GetGroup()->GetGroupType() != GROUP_TYPE_RAID)))
	...
	case ATTYPE_INSTANCE:
		{
			if(GetPlayer()->GetPlayerStatus() != TRANSFER_PENDING) //only ports if player is out of pendings
			{
				...
				if(reason != AREA_TRIGGER_FAILURE_OK)
				{
					...
					char msg[200];
					sStackWolrdPacket( data, SMSG_AREA_TRIGGER_MESSAGE, 250);
					data << uint32(0);
