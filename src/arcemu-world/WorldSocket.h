...
#define WORLDSOCKET_SENDBUF_SIZE 65384	//ugh, with 1000 clients, having each at least 5 of these buffest is like 500 MB ? Buffers : send, recv, update, compress, create... 
#define WORLDSOCKET_RECVBUF_SIZE 65384
...
public:
	...
	ARCEMU_INLINE void SendPacket(StackWorldPacket * packet) { if(!packet) return; OutPacket(packet->GetOpcode(), packet->GetSize(), (packet->GetSize() ? (const void*)packet->GetBufferPointer() : NULL)); }

	void __fastcall OutPacket(uint16 opcode, size_t len, const void* data, bool skip_compression = false);
...
private:
	...
	ByteBuffer	m_CompressedPacketsInputBuffer;	//dump packets here
	uint32		m_CompressedPacketsInBuffer;	//nr of packets
	uint8		m_CompressedPacketsOutBuffer[WORLDSOCKET_SENDBUF_SIZE];
	void		UpdateCompressedPackets(bool nolock=false);
#endif
};
...
static inline unsigned int FastGUIDPack(StackWorldPacket &packet,const uint64 & oldguid)
{
	int ret = FastGUIDPack(oldguid,packet.GetBufferPointer(),packet.GetSize());
	packet.AdvanceWrite( ret );
	return ret;
}
