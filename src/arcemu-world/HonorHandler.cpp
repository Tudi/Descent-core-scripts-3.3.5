...
void HonorHandler::OnPlayerKilledUnit( Player *pPlayer, Unit* pVictim )
{
	...
	if( Points > 0 )
	{
		pPlayer->Event_Achiement_Received(ACHIEVEMENT_CRITERIA_TYPE_HONORABLE_KILL,ACHIEVEMENT_UNUSED_FIELD_VALUE,ACHIEVEMENT_UNUSED_FIELD_VALUE,1,ACHIEVEMENT_EVENT_ACTION_ADD);
		pPlayer->Event_Achiement_Received(ACHIEVEMENT_CRITERIA_TYPE_HK_CLASS,pVictim->getClass(),ACHIEVEMENT_UNUSED_FIELD_VALUE,1,ACHIEVEMENT_EVENT_ACTION_ADD);
		pPlayer->Event_Achiement_Received(ACHIEVEMENT_CRITERIA_TYPE_HK_RACE,pVictim->getRace(),ACHIEVEMENT_UNUSED_FIELD_VALUE,1,ACHIEVEMENT_EVENT_ACTION_ADD);
		//also has map filter. But areas do not repeat over maps(unique id)
		pPlayer->Event_Achiement_Received(ACHIEVEMENT_CRITERIA_TYPE_HONORABLE_KILL_AT_AREA,pPlayer->GetAreaID(),ACHIEVEMENT_UNUSED_FIELD_VALUE,1,ACHIEVEMENT_EVENT_ACTION_ADD);
		if( pPlayer->m_bg )
		{
		...
		else
		{
			...
			for(Object::InRangeSet::iterator i = pPlayer->GetInRangeSetBegin(); i != pPlayer->GetInRangeSetEnd(); ++i)
