...
/*	size_t namelens;
	if(li)
		namelens = strlen(li->Name) + strlen(li->Description) + 137*4;
	else
		namelens = strlen(itemProto->Name1) + strlen(itemProto->Description) + 137*4;

	WorldPacket data(SMSG_ITEM_QUERY_SINGLE_RESPONSE, namelens );*/
	sStackWolrdPacket(data,SMSG_ITEM_QUERY_SINGLE_RESPONSE,sizeof(ItemPrototype)+256*4);
	...
	data << itemProto->existingduration;								// added in 2.4.2 duration(seconds) 0 = permanent
	data << itemProto->ItemLimitCategory;			// added in 3.0.2
	data << itemProto->HolydayId;	//from holyday.dbc
	data << itemProto->unk320_3;
	SendPacket( &data );
...
void WorldSession::HandleSellItemOpcode( WorldPacket & recv_data )
{
	...
	_player->Event_Achiement_Received(ACHIEVEMENT_CRITERIA_TYPE_GOLD_FROM_VENDORS,ACHIEVEMENT_UNUSED_FIELD_VALUE,ACHIEVEMENT_UNUSED_FIELD_VALUE,price,ACHIEVEMENT_EVENT_ACTION_ADD);
...
//	int8 bagslot = 0;
   ...
   if((error = _player->GetItemInterface()->CanAffordItem(it, amount, unit)) != 0)
   {
	...
	if(!add)
	{
		...
		itm->m_isDirty=true;
		itm->SetUInt32Value(ITEM_FIELD_STACK_COUNT, min(amount*item.amount,itm->GetProto()->MaxCount));
	...
	{
		add->ModUnsigned32Value(ITEM_FIELD_STACK_COUNT, min(amount*item.amount,add->GetProto()->MaxCount));
		add->m_isDirty = true;
...
void WorldSession::HandleBuyBankSlotOpcode(WorldPacket& recvPacket) 
{
	...
	if ((int32)_player->GetGold() >= price) 
	{
	   _player->SetUInt32Value(PLAYER_BYTES_2, (bytes&0xff00ffff) | ((slots+1) << 16) );
	   _player->ModGold( -price);
	}
	_player->Event_Achiement_Received( ACHIEVEMENT_CRITERIA_TYPE_BUY_BANK_SLOT, ACHIEVEMENT_UNUSED_FIELD_VALUE,ACHIEVEMENT_UNUSED_FIELD_VALUE, 1, ACHIEVEMENT_EVENT_ACTION_ADD );
}
...
//			if (apply) 
			{
				...
				// Skill requirement
				if( ip->RequiredSkill && ip->RequiredSkillRank > _player->_GetSkillLineCurrent( ip->RequiredSkill, true ) )
				{
...
/*				if(!(gp->SocketMask & TargetItem->GetProto()->Sockets[i].SocketColor))
				{
					//check if we are inserting it to an enchantment created slot. 
					//FIXME : ! need to link new gem to enchantment to not be able to insert multiple gems into same slot !
					bool found_match = false;
					for( uint32 tSlot = ITEM_ENCHANT_SLOT_RANDOM_SUFFIX1; tSlot <= ITEM_ENCHANT_SLOT_RANDOM_PROP3; tSlot++ )
					{
						EnchantmentInstance * tEI = TargetItem->GetEnchantment( tSlot );
						//check if adds an extra socket
						if( tEI == NULL )
							continue;
						bool ads_socket = false;
						for(int i=0;i<3;i++)
							if( tEI->Enchantment->type[i] == SPELL_ENCHANT_TYPE_ADD_SOCKET )
							{
								ads_socket = true;
								break;
							}
						//check if it matches color
						if( ads_socket )
						{
//							for(int i=0;i<3;i++)
//								//really not sure about this. it seemed to match this formula for the only prismatic gem example there was available
//								if( ( 1 << tEI->Enchantment->min[ i ] ) & gp->SocketMask )
//								{
//									found_match = true;
//									break;
//								}
							found_match = true; //no idea where the mask can be found. Trust the client i guess. Sockets are meta.
						}
					}

					if( found_match == false )
					{
						itemi->BuildInventoryChangeError(it, TargetItem, INV_ERR_ITEM_DOESNT_GO_TO_SLOT);
						continue;
						ColorMatch=false;
					}
				}*/
...
void WorldSession::HandleEquipmentSetSave(WorldPacket &recv_data)
{
	...
	sStackWolrdPacket( data, SMSG_EQUIPMENT_SET_SAVED, 4 + 9 + 1 );
	data << uint32(index);
...
void WorldSession::HandleEquipmentSetUse(WorldPacket &recv_data)
{
    ...
    for(uint32 item_slot = 0; item_slot < EQUIPMENT_SLOT_END; ++item_slot)
    {
		...
		if( new_item )
		{
			AddItemResult result = (AddItemResult)_player->GetItemInterface()->CanEquipItemInSlot2( INVENTORY_SLOT_NOT_SET, item_slot, new_item );
			if( result == INV_ERR_OK )
    ...
    sStackWolrdPacket( data, SMSG_EQUIPMENT_SET_USE_RESULT, 2 );
