...
void MoonScriptCreatureAI::SetDisplayWeapon(bool pMainHand, bool pOffHand)
{
	...
	{
		if( pMainHand )
			disp1 = _unit->m_spawn->Item1SlotDisplay;
		if( pOffHand )
			disp1 = _unit->m_spawn->Item2SlotDisplay;
	}
	SetDisplayWeaponIds( disp1 , disp2 );
}
...
void MoonScriptCreatureAI::AIUpdate()
{
	...
	for( SpellDescList::iterator SpellIter = mScheduledSpells.begin(); SpellIter != mScheduledSpells.end(); )
	{
		...
		{
			//maybe spell got deleted whyle we casted it ? This is trying to be a crashfix 
			for( SpellDescList::iterator SpellIter2 = mScheduledSpells.begin(); SpellIter2 != mScheduledSpells.end();SpellIter2++ )
				if( Spell == (*SpellIter2) )
				{
					mScheduledSpells.erase( SpellIter2 );
					break;
				}
			break;
		}
...
//		for( std::set<Object*>::iterator ObjectIter = _unit->GetInRangeOppFactsSetBegin(); ObjectIter != _unit->GetInRangeOppFactsSetEnd(); ++ObjectIter )
		...
		{
			if( IsValidUnitTarget(*itr, pFilter) ) TargetArray.push_back(static_cast<Unit*>(*itr));
		}
	}
