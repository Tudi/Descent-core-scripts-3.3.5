...
class AshtongueSpiritbinderAI : public MoonScriptCreatureAI
{
	...
	AshtongueSpiritbinderAI(Creature *pCreature) : MoonScriptCreatureAI(pCreature)
	{
		...
		if (_unit->GetMapMgr()->instance_difficulty != INSTANCE_MODE_RAID_25_MAN_HEROIC)	// Guessed
...
class AkamaAI : public MoonScriptBossAI
{
	...
	void ToIllidanEvent()
	{
		...
		Unit *pDoorTrigger = ForceCreatureFind(CN_DOOR_EVENT_TRIGGER, 771.5f, 304.7f, 319.0f, false);
	...
	void IllidanDialogEvent()
	{
		if (mIllidanAI == NULL)
		{
			Creature *pIllidan = (Creature*)(ForceCreatureFind(CN_ILLIDAN_STORMRAGE, 704.539001f, 305.282013f, 353.919006f));
	...
	void EncounterEnd()
	{
		...
		case 1:{
			...
			Unit *pIllidan = ForceCreatureFind(CN_ILLIDAN_STORMRAGE);
			if (pIllidan != NULL)
			{
	...
	void FightWithIllidan()
	{
		...
		if (mIllidanAI->IsAlive())
		{
			...
			else if (IsTimerFinished(mTrapTimer))
			{
				...
				{
					CastSpellNowNoScheduling(mTeleport);
					static_cast<Object*>(_unit)->SetPosition(pTarget->GetPositionX(), pTarget->GetPositionY(), pTarget->GetPositionZ(), pTarget->GetOrientation());		// does it really work?
	...
	void DeathScene()
	{
		...
		if ((mIllidanAI != NULL && mScenePart <= 6) || mScenePart > 6)
		{
			...
			case 6:{
				...
				Creature *pAkama = static_cast<Creature*>(ForceCreatureFind(CN_AKAMA, ToIllidan[19].mX, ToIllidan[19].mY, ToIllidan[19].mZ, false));
				if (pAkama != NULL)
				{
...
class IllidanStormrageAI : public MoonScriptBossAI
{
	...
	IllidanStormrageAI(Creature* pCreature) : MoonScriptBossAI(pCreature)
	{
		...
		_unit->SetUInt32Value(UNIT_FIELD_BASEATTACKTIME_1, 1800);
		_unit->SetUInt32Value(UNIT_NPC_EMOTESTATE, 0);
		_unit->SetDualWield(true);
	...
	void OnCombatStop(Unit * pTarget)
	{
		...
		Creature *pTrigger = (Creature*)(ForceCreatureFind(CN_FACE_TRIGGER, 677.399963f, 305.545044f, 353.192169f, false));
		if (pTrigger != NULL)
		{
		...
		for (int i = 0; i < 2; ++i)
		{
			Creature *pBlade = static_cast<Creature*>(ForceCreatureFind(CN_BLADE_OF_AZZINOTH, UnitPos[i].mX, UnitPos[i].mY, UnitPos[i].mZ, false));
			if (pBlade != NULL)
			{
	...
	void PhaseOne()
	{
		...
		{
			Creature *pAkama = (Creature*)(ForceCreatureFind(CN_AKAMA));
			if (pAkama != NULL && pAkama->GetScript() != NULL)
			{
				...
				if (pAkamaAI->mScenePart <= 2 && pAkamaAI->GetCanEnterCombat())
				{
					...
					pAkamaAI->GetUnit()->GetAIInterface()->SetNextTarget((Unit*)NULL);
	...
	void PhaseTwo()
	{
		if (mScenePart != 0)
		{
			...
			case 2:
				for (int i = 0; i < 2; ++i)
				{
					Creature *pBlade = _unit->GetMapMgr()->GetInterface()->SpawnCreature(CN_BLADE_OF_AZZINOTH, UnitPos[i].mX, UnitPos[i].mY, UnitPos[i].mZ, UnitPos[i].mO, false, false, 0, 0);
					if (pBlade != NULL)
					{
			...
			case 3:
				...
				if (mFoA1 != NULL)
				{
					Unit *pBlade = ForceCreatureFind(CN_BLADE_OF_AZZINOTH, UnitPos[0].mX, UnitPos[0].mY, UnitPos[0].mZ, false);
					if (pBlade != NULL)
					{
			...
			case 5:
				for (int i = 0 ; i < 2; ++i)
				{
					Creature *Blade = static_cast<Creature*>(ForceCreatureFind(CN_BLADE_OF_AZZINOTH, UnitPos[i].mX, UnitPos[i].mY, UnitPos[i].mZ, false));
					if (Blade != NULL)
					{
						Blade->Despawn(0,0);
					}
		...
		if (mFoA2 != NULL)
		{
			if (!mFoA2->IsAlive())
			{
				Unit *pBlade = ForceCreatureFind(CN_BLADE_OF_AZZINOTH, UnitPos[1].mX, UnitPos[1].mY, UnitPos[1].mZ, false);
				if (pBlade != NULL)
				{
					pBlade->SetUInt64Value(UNIT_FIELD_CHANNEL_OBJECT, 0);
					pBlade->SetUInt32Value(UNIT_CHANNEL_SPELL, 0);
		...
		if (GetMoveType() != Move_WantedWP && !IsCasting())
		{
			if (_unit->GetUInt32Value(UNIT_CHANNEL_SPELL) == 0)
			{
				...
				{
					Unit *pTrigger = ForceCreatureFind(CN_FACE_TRIGGER, 677.399963f, 305.545044f, 353.192169f, false);
					if (pTrigger != NULL)
					{
			...
			if (mMiscEventPart != 0)
			{
				if (mMiscEventPart == 1)
				{
					...
					Creature *pTrigger = _unit->GetMapMgr()->GetInterface()->SpawnCreature(CN_EYE_BEAM_TRIGGER, EyeBeamPaths[FireWall].mX, EyeBeamPaths[FireWall].mY, EyeBeamPaths[FireWall].mZ, EyeBeamPaths[FireWall].mO, false, false, 0, 0);
					if (pTrigger != NULL && pTrigger->GetScript() != NULL)
					{
						...
						EyeBeamTriggerAI *pEyeBeamTriggerAI = (EyeBeamTriggerAI*)(static_cast<MoonScriptCreatureAI*>(((Creature*)(pTrigger))->GetScript()));
...
class CageTrapTriggerAI : public MoonScriptCreatureAI
{
    ...
    void AIUpdate()
    {
		...
		if (pIllidan != NULL)
		{
			IllidanStormrageAI *pAI = (IllidanStormrageAI*)(static_cast<MoonScriptCreatureAI*>(static_cast<Creature*>(pIllidan)->GetScript()));
...
class DranaeiSpiritAI : public MoonScriptCreatureAI
{
	...
	void Destroy()
	{
		Creature *pAkama = (Creature*)(ForceCreatureFind(22990));
		if (pAkama != NULL && pAkama->GetScript() != NULL)
		{
			AkamaAI *pAI = (AkamaAI*)(pAkama->GetScript());
			if (pAI != NULL)
...
class FlameOfAzzinothAI : public MoonScriptCreatureAI
{
	...
	void AIUpdate()
	{
		...
		Unit *pBlade1 = ForceCreatureFind(22996, UnitPos[0].mX, UnitPos[0].mY, UnitPos[0].mZ, false);
		Unit *pBlade2 = ForceCreatureFind(22996, UnitPos[1].mX, UnitPos[1].mY, UnitPos[1].mZ, false);
	...
	void Destroy()
	{
		Creature *pIllidan = (Creature*)(ForceCreatureFind(CN_ILLIDAN_STORMRAGE));
		if (pIllidan != NULL && pIllidan->GetScript() != NULL)
		{
