...
RegType<Unit> UnitMethods[] = {
	...
	{ "CheckSpell", &luaUnit_UnlearnSpell },
...
//	{ "AddAssistTarget", &luaUnit_AddAssistTargets },
	...
	{ "SetUInt32Variable", &luaObject_SetExtensionUInt32 },
	{ "ModUInt32Variable", &luaObject_ModExtensionUInt32 },
	{ "GetUInt32Variable", &luaObject_GetExtensionUInt32 },
	{ "SetMapUInt32Variable", &luaObject_SetMapExtensionUInt32 },
	{ "ModMapUInt32Variable", &luaObject_ModMapExtensionUInt32 },
	{ "GetMapUInt32Variable", &luaObject_GetMapExtensionUInt32 },
	{ "PlayCinematic", &luaUnit_PlayCinematic },
	{ "CastAllMapPlayers", &luaUnit_CastAllMapPlayers },
	{ "GetIstanceDifficulty", &luaUnit_GetInstanceDiffMode },
	//oh crap these are not 64 bit variables after all :( Just putting them here for compatibility for now !
	{ "SetUInt64Variable", &luaObject_SetExtensionUInt32 },
	{ "ModUInt64Variable", &luaObject_ModExtensionUInt32 },
	{ "GetUInt64Variable", &luaObject_GetExtensionUInt32 },
...
RegType<Object> ObjectMethods[] = {
	{ "AddUInt32Variable", &luaObject_SetExtensionUInt32 },
	{ "ModUInt32Variable", &luaObject_ModExtensionUInt32 },
	{ "GetUInt32Variable", &luaObject_GetExtensionUInt32 },
//	{ "AddFloatVariable",  &luaObject_AddExtensionFloat },
//	{ "GetFloatVariable",  &luaObject_GetExtensionFloat },
	{ NULL, NULL },
};

template<typename T> RegType<T>* GetMethodTable() { return NULL; }

template<> RegType<Object>* GetMethodTable<Object>() { return ObjectMethods; }
...
public:
	...
	void StringFunctionCallStr(const char * pFunction)
	{
...
{
	if(ptr == NULL || ptr->GetMapMgr() == NULL || ptr->GetMapMgr()->GetInterface() == NULL)
		return 0;
...
int luaUnit_RegisterEvent(lua_State * L, Unit * ptr)
{
	...
	sEventMgr.AddEvent(pCreature, &Creature::TriggerScriptEventStr, strFunc, EVENT_CREATURE_UPDATE, (uint32)delay, (uint32)repeats, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
	return 0;
}
...
{
	if( IS_STATIC_MAP( ptr->GetMapMgr()->GetMapInfo() ) )
		lua_pushnil(L);
	else
...
int luaUnit_GetInRangeFriends(lua_State * L, Unit * ptr)
{
	...
	for( set<Object*>::iterator itr = ptr->GetInRangeSetBegin(); itr != ptr->GetInRangeSetEnd(); itr++)
	{
		if( (*itr)->IsUnit() == false || static_cast<Unit*>(*itr)->isAlive() == false || isFriendly( (*itr), ptr ) == false )
			continue;
		count++,
		pC = *itr;
...
int luaUnit_CheckSpell(lua_State * L, Unit* ptr)
{
	CHECK_TYPEID(TYPEID_PLAYER);
	int spellid = luaL_checkint(L,1);
	if(((Player*)ptr)->HasSpell(spellid))
		lua_pushboolean(L,1);
	else
		lua_pushboolean(L,0);
	return 1;
...
{
	if(ptr && ptr->GetCurrentSpell() != NULL)
	{
...
int luaUnit_CreateGuardian(lua_State * L, Unit * ptr)
{
	...
	if(!ptr||!guardian_entry||!lvl)
		return 0;
	ptr->create_guardian(guardian_entry,duration);
	return 0;
}
...
int luaUnit_GetUnitByGUID(lua_State * L, Unit * ptr)
{
	...
	//the complicated fetching is only required when some smartass truncated 64 bit GUID to 32(like lua getguid)
	//this is a very bad solution cause players and mobs can have same lowguid
	if(ptr && guid)
	{
		//sucks if guid is player or pet
		Unit *pp = ptr->GetMapMgr()->GetPlayer(guid);
		if( pp == NULL )
		{
			Unit *pc = ptr->GetMapMgr()->GetCreature(guid);
			if( pc == NULL )
			{
				Unit *ppet = ptr->GetMapMgr()->GetPet(guid);
...
int luaUnit_SetMovementFlags(lua_State * L, Unit * ptr)
	{
		if(ptr == NULL || !ptr->IsInWorld() || ptr->GetTypeId() != TYPEID_UNIT) { return 0; }
...
{
	if(ptr == NULL || !ptr->IsInWorld() || ptr->GetTypeId() != TYPEID_PLAYER) { return 0; }
		bool lock = (lua_toboolean((L),(1)) > 0) ? true : false;
		if (lock) 
		{
			ptr->m_pacified = 1;
...
int luaUnit_PlayCinematic(lua_State * L, Unit * ptr)
{
	if(ptr == NULL || !ptr->IsInWorld() || ptr->IsPlayer() == false) { return 0; }
	uint32 movie = luaL_checkint(L, 1);
	static_cast<Player*>( ptr )->SendTriggerMovie( movie );
	return 0;
}

int luaUnit_CastAllMapPlayers(lua_State * L, Unit * ptr)
{
	if(ptr == NULL || !ptr->IsInWorld() || ptr->IsPlayer() == false) { return 0; }
	uint32 cast_id = luaL_checkint(L, 1);
	PlayerStorageMap babkuplist( ptr->GetMapMgr()->m_PlayerStorage );
	PlayerStorageMap::iterator itr;
	for(itr = babkuplist.begin(); itr != babkuplist.end();++itr)
		itr->second->CastSpell(itr->second,cast_id,true);
	return 0; //returning 0 nr of values
}

int luaUnit_GetInstanceDiffMode(lua_State * L, Unit * ptr)
{
	if(ptr == NULL || !ptr->IsInWorld())
		return 0;
	lua_pushinteger(L,ptr->GetMapMgr()->instance_difficulty );
	return 1; //returning 0 nr of values
}
...
int luaUnit_MovePlayerTo(lua_State * L, Unit * ptr)
	{
		if(ptr == NULL || !ptr->IsInWorld() || ptr->GetTypeId() != TYPEID_UNIT)
		{
			return 0;
		}
		float x = (lua_isnoneornil(L,1) ) ? 0.00f : (float)luaL_checknumber(L,1);
		float y = (lua_isnoneornil(L,2) ) ? 0.00f : (float)luaL_checknumber(L,2);
		float z = (lua_isnoneornil(L,3) ) ? 0.00f : (float)luaL_checknumber(L,3);
		float o = (lua_isnoneornil(L,4) ) ? 0.00f : (float)luaL_checknumber(L,4);
		uint32 mov_flag = (uint32)luaL_checknumber((L),(5));//0 - walk, 256 - teleport, 4096 - run, 12288 - fly
...
////////////////////////////////////////////////
// UNIT - OBJECT get set extension
////////////////////////////////////////////////

enum ExtensionStoreTypes
{
	EXTENSION_STORE_TYPE_UINT32	= 1,
	EXTENSION_STORE_TYPE_GUID	= 2,
	EXTENSION_STORE_TYPE_Float	= 3,
};

class extension_uint32_store:public BaseObjectExtension
{
public:
	uint32 val;
};

class extension_GUID_store:public BaseObjectExtension
{
public:
	uint64 val;
};

class extension_Float_store:public BaseObjectExtension
{
public:
	float val;
};

int luaObject_SetExtensionUInt32(lua_State * L, Object * ptr)
{
	const char *variable_name = luaL_checkstring(L, 1);
	if( variable_name == NULL )
		return 0;
	uint32 variable_val = luaL_checkint(L, 2);

	extension_uint32_store *t = (extension_uint32_store *)ptr->GetExtension( variable_name );
	if( t == NULL)
	{
		t = new extension_uint32_store;
		t->type = EXTENSION_STORE_TYPE_UINT32;
		ptr->SetExtension( variable_name, t );
	}
	if( t->type == EXTENSION_STORE_TYPE_UINT32 )
		t->val = variable_val;

	return 1;
}

int luaObject_ModExtensionUInt32(lua_State * L, Object * ptr)
{
	const char *variable_name = luaL_checkstring(L, 1);
	if( variable_name == NULL )
		return 0;
	int32 variable_val = luaL_checkint(L, 2);

	extension_uint32_store *t = (extension_uint32_store*)ptr->GetExtension( variable_name );
	if( t != NULL && t->type == EXTENSION_STORE_TYPE_UINT32 )
		t->val += variable_val;

	return 1;
}

int luaObject_GetExtensionUInt32(lua_State * L, Object * ptr)
{
	const char *variable_name = luaL_checkstring(L, 1);
	if( variable_name == NULL )
		return 0;
	...
	extension_uint32_store *t = (extension_uint32_store*)ptr->GetExtension( variable_name );
	if( t != NULL && t->type == EXTENSION_STORE_TYPE_UINT32 )
		lua_pushinteger(L,t->val);
	else
		return 0;
	return 1;
}

int luaObject_SetMapExtensionUInt32(lua_State * L, Unit * ptr)
{
	if( ptr == NULL || ptr->GetMapMgr() == NULL )
		return 0;
	const char *variable_name = luaL_checkstring(L, 1);
	if( variable_name == NULL )
		return 0;
	...
	extension_uint32_store *t = (extension_uint32_store *)ptr->GetMapMgr()->GetExtension( variable_name );
	if( t == NULL)
	{
		t = new extension_uint32_store;
		t->type = EXTENSION_STORE_TYPE_UINT32;
		ptr->GetMapMgr()->SetExtension( variable_name, t );
	}
	if( t->type == EXTENSION_STORE_TYPE_UINT32 )
		t->val = variable_val;
...
int luaObject_ModMapExtensionUInt32(lua_State * L, Unit * ptr)
{
	if( ptr == NULL || ptr->GetMapMgr() == NULL )
		return 0;
	const char *variable_name = luaL_checkstring(L, 1);
...
int luaObject_GetMapExtensionUInt32(lua_State * L, Unit * ptr)
{
	if( ptr == NULL || ptr->GetMapMgr() == NULL )
		return 0;
...
/*
int luaObject_AddExtensionFloat(lua_State * L, Object * ptr)
{
	int variable_id = luaL_checkint(L, 1);
	float variable_val = (float)luaL_checknumber(L, 2);

	extension_Float_store *t = (extension_Float_store *)ptr->GetExtension( variable_id );
	if( t == NULL)
	{
		t = new extension_Float_store;
		t->type = EXTENSION_STORE_TYPE_Float;
		ptr->SetExtension( variable_id, t );
	}
	if( t->type == EXTENSION_STORE_TYPE_Float )
		t->val = variable_val;

	return 1;
}

int luaObject_GetExtensionFloat(lua_State * L, Object * ptr)
{
	int variable_id = luaL_checkint(L, 1);

	extension_Float_store *t = (extension_Float_store*)ptr->GetExtension( variable_id );
	if( t != NULL && t->type == EXTENSION_STORE_TYPE_Float )
		lua_pushnumber(L,t->val);

	return 1;
}*/

int luaObject_SetExtensionUInt32(lua_State * L, Unit * ptr)
{
	return luaObject_SetExtensionUInt32(L,(Object*)ptr);
}

int luaObject_ModExtensionUInt32(lua_State * L, Unit * ptr)
{
	return luaObject_ModExtensionUInt32(L,(Object*)ptr);
}

int luaObject_GetExtensionUInt32(lua_State * L, Unit * ptr)
{
	return luaObject_GetExtensionUInt32(L,(Object*)ptr);
}

/*
int luaObject_AddExtensionFloat(lua_State * L, Unit * ptr)
{
	return luaObject_AddExtensionFloat(L,(Object*)ptr);
}

int luaObject_GetExtensionFloat(lua_State * L, Unit * ptr)
{
	return luaObject_GetExtensionFloat(L,(Object*)ptr);
}*/
