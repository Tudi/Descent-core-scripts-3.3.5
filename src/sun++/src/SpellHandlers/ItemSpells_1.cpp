...
bool ForemansBlackjack(uint32 i, Spell *pSpell)
{
	...
	//wakeup sound 
	pSpell->p_caster->PlaySoundTarget( 6197, c_target->GetGUID() );
...
bool BarkOfTheWalkers(uint32 i, Spell * pSpell)
{
	if( !pSpell->p_caster )
		return true;

	Unit * target = pSpell->GetUnitTarget();
	if( !target || target->GetEntry() != 26321 )
		return true;

	Player * pPlayer = pSpell->p_caster;
	QuestLogEntry * en = pPlayer->GetQuestLogForEntry( 12096 );
	if( en && en->GetMobCount( 0 ) < en->GetQuest()->required_mobcount[0] )
	{
		en->SetMobCount( 0, en->GetMobCount( 0 ) + 1 );
		en->SendUpdateAddKill( 0 );
		en->UpdatePlayerFields();
		((Creature*)target)->SendChatMessage( CHAT_MSG_MONSTER_SAY, LANG_UNIVERSAL, "The Lothalor Ancient gives you its thanks." );
		((Creature*)target)->Despawn( 500, 360000 );
	}
	return true;
}
...
void SetupItemSpells_1(ScriptMgr * mgr)
{
	...
	mgr->register_dummy_spell(35036, &BarkOfTheWalkers);		// Give to the Lothalor Ancients to strengthen them
