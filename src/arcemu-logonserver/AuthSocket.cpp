...
void AuthSocket::HandleChallenge()
{
	...
	{
		sLog.outDebug("HandleChallenge : Packet too small, exiting");
		return;
	}
	...
	{
		sLog.outDebug("HandleChallenge : Packet too large, exiting");
		Disconnect();
		return;
	}
...
//#endif
	...
	{
		// wtf?
		sLog.outDebug("HandleChallenge : Client too new(%u>%u), exiting",build,LogonServer::getSingleton().max_build);
	...
	if(build < LogonServer::getSingleton().min_build)
	{
		...
		if(m_patch == NULL)
		{
			...
			sLog.outDebug("HandleChallenge : Client too old, could not find patch, exiting");
			SendChallengeError(CE_WRONG_BUILD_NUMBER);
			return;
		}
		...
		Send(response, 119);
		sLog.outDebug("HandleChallenge : Client too old, exiting");
		return;
	}
	...
	switch(ipb)
	{
		case BAN_STATUS_PERMANENT_BAN:
			sLog.outDebug("HandleChallenge : IP ban, exiting");
			SendChallengeError(CE_ACCOUNT_CLOSED);
			return;

		case BAN_STATUS_TIME_LEFT_ON_BAN:
			sLog.outDebug("HandleChallenge : Acc ban, exiting");
	...
	{
		sLog.outDebug("HandleChallenge : Missing string length, exiting");
		Disconnect(); 
		return; 
	}
	...
	if( i != string::npos )
	{
		sLog.outDebug("HandleChallenge : Invalid acc string, exiting");
		return;
		//AccountName.erase( i );
	}
	...
	{
		SendChallengeError(CE_ACCOUNT_CLOSED);
		sLog.outDebug("HandleChallenge : Acc banned, exiting");
		return;
	}
	...
	uint8 security_flags = 0;
	response[c] = security_flags;							c += 1;
	if(security_flags & 0x01)                // PIN input
	{
		memset(&response[c],0,4+8+8);						c += 4+8+8;
	}
	if(security_flags & 0x02)                // Matrix input
	{
		memset(&response[c],0,1+1+1+1+8);					c += 1+1+1+1+8;
	}
	if(security_flags & 0x04)                // Security token input
	{
		response[c] = 1;										c += 1;
	}
	Send(response, c);
