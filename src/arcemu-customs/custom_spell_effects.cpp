	...
	case 4014:
		{
			...
			int cost_to_function = m_spellInfo->EffectBasePoints[0];
...
/*			if( m_spellInfo->EffectBaseDice[0] && (int)p_caster->GetUInt32Value( PLAYER_FIELD_COINAGE ) >= cost_to_function * m_spellInfo->EffectBaseDice[0] )
			...
			if( m_spellInfo->EffectBasePoints[1] && (int)p_caster->GetUInt32Value( PLAYER_FIELD_COINAGE ) >= cost_to_function )
			{
				...
				if( p && p->GetUInt32Value(UNIT_FIELD_HEALTH) < p->GetUInt32Value(UNIT_FIELD_MAXHEALTH) )
				{
					...
					uint32 add = cur * m_spellInfo->EffectBasePoints[1] / 100;
			...
			if( m_spellInfo->EffectBasePoints[2] && (int)p_caster->GetUInt32Value( PLAYER_FIELD_COINAGE ) >= cost_to_function )
			{
				uint32 cur = p_caster->GetUInt32Value(UNIT_FIELD_MAXPOWER1);
				uint32 add = cur * m_spellInfo->EffectBasePoints[2] / 100;
