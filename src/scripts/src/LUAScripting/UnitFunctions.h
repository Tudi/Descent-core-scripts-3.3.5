...
namespace luaUnit
{
	...
	int AggroWithInRangeFriends(lua_State * L, Unit * ptr)
	{
		...
		{
			Object * obj = static_cast<Object*>(*itr);
			// No Object, Object Isn't a Unit, Unit is Dead
			if (!obj || !obj->IsUnit() || static_cast<Unit*>(obj)->IsDead())
			   continue;
...
/*		std::map< uint64,std::set<int> > & objRefs = sLuaMgr.getObjectFunctionRefs();
	...
	{
		//TEST_UNIT()
		if(!ptr || ptr->GetMapMgr() == NULL || IS_STATIC_MAP( ptr->GetMapMgr()->GetMapInfo() ) )
			lua_pushnil(L);
		else
	...
	int GetClosestPlayer(lua_State * L, Unit * ptr)
	{
		...
		for(set< Object* >::iterator itr = ptr->GetInRangeSetBegin(); itr != ptr->GetInRangeSetEnd(); ++itr)
		{
			if( (*itr)->IsPlayer() == false || static_cast<Unit*>((*itr))->isAlive() == false )
				continue;
	...
	int GetRandomFriend(lua_State * L, Unit * ptr)
	{
		...
		{
			Object* obj = static_cast<Object*>(*itr);
			if (obj->IsUnit() && isFriendly(obj,ptr) && static_cast<Unit*>(obj)->isAlive() )
	...
	int GetRandomEnemy(lua_State * L, Unit * ptr)
	{
		...
		{
			Object* obj = static_cast<Object*>(*itr);
			if (obj->IsUnit() && isHostile(ptr,obj) && static_cast<Unit*>(obj)->isAlive() )
...
//								item->SetStackCount( qst->srcitemcount ? qst->srcitemcount : 1);
	...
	*/
	...
	int Kill(lua_State * L, Unit * ptr)
	{
		...
		if (!ptr || !target)
			return 0;
		ptr->SetUInt32Value(UNIT_FIELD_HEALTH,1);
	...
	int CreateGuardian(lua_State * L, Unit * ptr)
	{
		...
		if (!ptr || !guardian_entry || !lvl)
			return 0;
		PUSH_UNIT(L, static_cast<Unit*>(ptr->create_guardian(guardian_entry,duration,lvl,NULL)));
		return 1;
	}
...
/*				if (plr->GetGroup()->GetGroupType() == GROUP_TYPE_PARTY)
					lua_pushnumber(L,plr->GetGroup()->instance_difficulty);
				else
					lua_pushnumber(L,plr->GetGroup()->m_raiddifficulty);*/
				lua_pushnumber(L,plr->GetInstanceDifficulty() );
			}
			else
			{
				if (!plr->IsInInstance())
				...
				lua_pushinteger(L,pInstance->instance_difficulty);
			}
			return 1;
		}
		else
		...
		*/
		...
		if (ptr->IsInInstance()) 
		{
			if (ptr->IsPlayer())
			{
				Player * plr = static_cast<Player*>(ptr);
				if (plr->GetGroup())
					(difficulty > 1 ? plr->GetGroup()->dungeon_difficulty : plr->GetGroup()->raid_difficulty) = difficulty;
				else
				{
					Instance * pInstance = sInstanceMgr.GetInstanceByIds(plr->GetMapId(), plr->GetInstanceID());
					pInstance->instance_difficulty = difficulty;
				}
			}
			else
			{
	...
	int SetCreatureName(lua_State * L, Unit * ptr)
	{
		...
		if(!ptr|!id)
			return 0;
		static_cast<Player*>(ptr)->SetCreatureInfo(CreatureNameStorage.LookupEntry(id));
		*/
		return 0;
	}
	...
	int AddAura(lua_State * L, Unit * ptr)
	{
		...
		if(ptr && spellid)
		{
			...
			aura->Init( dbcSpell.LookupEntry(spellid), duration, ptr, ptr);
//			Aura * aura = new Aura(dbcSpell.LookupEntry(spellid), duration, ptr, ptr,temp);
	...
	int ModifyAIUpdateEvent(lua_State * L, Unit * ptr)
	{
		...
		sEventMgr.ModifyEventRepeatInterval(ptr,EVENT_SCRIPT_UPDATE_EVENT,newtime);
		sEventMgr.ModifyEventTimeLeft(ptr,EVENT_SCRIPT_UPDATE_EVENT,newtime);
		return 0;
	}
	...
	int HasTitle(lua_State * L, Unit * ptr)
	{
		...
		int title = luaL_checkint(L, 1);
		Player * plr = static_cast<Player*>(ptr);
		if (plr->HasKnownTitle(RankTitles(title)) )
			lua_pushboolean(L,1);
		else
			lua_pushboolean(L,0);
		return 1;
...
//			CreatureProto * proto = static_cast<Creature*>(pUnit)->GetProto();
			CreatureProto * proto = static_cast<Creature*>(pUnit)->DificultyProto;
			switch (loot_type) 
			{
				default:
//					lootmgr.FillCreatureLoot(&pUnit->loot, pUnit->GetEntry(), pUnit->GetMapMgr() ? ( pUnit->GetMapMgr()->instance_difficulty ) : false);
					if( proto && proto->loot_kill )
						lootmgr.FillObjectLoot(pUnit,proto->loot_kill);
					pUnit->loot.gold = proto ? proto->money : 0;
					loot_type2 = 1;
					break;
				case 2:
//					lootmgr.FillSkinningLoot(&pUnit->loot, pUnit->GetEntry());
					if( proto && proto->loot_skin )
						lootmgr.FillObjectLoot(pUnit,proto->loot_skin);
					loot_type2 = 2;
					break;
				case 3:
//					lootmgr.FillPickpocketingLoot(&pUnit->loot, pUnit->GetEntry());
					if( proto && proto->loot_pickpocket )
						lootmgr.FillObjectLoot(pUnit,proto->loot_pickpocket);
					loot_type2 = 2;
					break;
			}
		}
		else if (guidtype == HIGHGUID_TYPE_GAMEOBJECT)
		{
			...
			{
				default:
					lootmgr.FillGOLoot(&pGO->loot, pGO->GetEntry(), pGO->GetMapMgr() ? ( pGO->GetMapMgr()->instance_difficulty ) : false);
					loot_type2 = 1;
					break;
/*				case 5:
...
//			sp->CanFly = 0;
			...
			Creature * p = ptr->GetMapMgr()->CreateCreature(entry);
			ASSERT(p);
...
/*
	...
	{
		TEST_UNIT()
		lua_pushboolean(L, ( static_cast<Creature*>(ptr)->IsVehicle() ) ? 1 : 0);
		return 1;
	}
...
//		static_cast<Player*>(ptr)->m_specs[spec].m_customTalentPointOverride = points;
		static_cast<Player*>(ptr)->m_Talent_point_mods = points;
		return 0;
	}
...
/*		PlayerSpec plrSpec = static_cast<Player*>(ptr)->m_specs[spec];
		...
		lua_pushnumber(L,static_cast<Player*>(ptr)->GetUInt32Value(PLAYER_CHARACTER_POINTS1));
		return 1;
	}
	...
	int AddAchievement(lua_State * L, Unit * ptr)
	{
		...
		Player * plr = static_cast<Player*>(ptr);
		if (plr->Achiement_Finished_Add( achievementID ) )
			lua_pushboolean(L, 1);
		else
			lua_pushboolean(L, 0);
		return 1;
	...
	int RemoveAchievement(lua_State * L, Unit * ptr)
	{
		TEST_PLAYER()
		int32 achievementID = luaL_checkint(L,1);
		static_cast<Player*>(ptr)->Achiement_Remove(achievementID);
		return 0;
	}
	...
	int HasAchievement(lua_State * L, Unit * ptr)
	{
		...
		lua_pushboolean(L, static_cast<Player*>(ptr)->Achiement_Has_Finished(achievementID) ? 1 : 0);
		return 1;
	}
	...
	int GetAura(lua_State * L, Unit * ptr)
	{
		...
		if (slot >= FIRST_AURA_SLOT && slot < MAX_PASSIVE_AURAS)
	...
	{
		TEST_UNITPLAYER_RET();
		for (uint32 x = MAX_NEGATIVE_VISUAL_AURAS_START; x < MAX_NEGATIVE_AURAS1(ptr); ++x)
		{
	...
	{
		TEST_UNITPLAYER()
		for (uint32 x = MAX_POSITIVE_VISUAL_AURAS_START; x < MAX_PASSIVE_AURAS1(ptr); ++x)
		{
			if (ptr->m_auras[x] && ptr->m_auras[x]->m_spellProto)
				RET_BOOL(true)
		}
...
//		dungeon->SaveToDB();
	...
	{
		TEST_UNITPLAYER_RET();
		if(ptr->GetMapMgr()->GetMapInfo() && IS_MAP_DUNGEON( ptr->GetMapMgr()->GetMapInfo() ) )
			lua_pushboolean(L,1);
		else
			lua_pushboolean(L,0);
		return 1;
	...
	{
		TEST_UNITPLAYER_RET();
		if(ptr->GetMapMgr()->GetMapInfo() && IS_MAP_RAID( ptr->GetMapMgr()->GetMapInfo() ))
			lua_pushboolean(L,1);
		else
			lua_pushboolean(L,0);
		return 1;
	...
	int GetAuraStackCount(lua_State * L, Unit * ptr)
	{
		TEST_UNITPLAYER_RET();
		uint32 id = CHECK_ULONG(L,1);
		Aura *a = ptr->FindAura(id);
		if( a == NULL )
			RET_NUMBER( 0 )
		else
			RET_NUMBER(ptr->m_auraStackCount[ a->m_visualSlot ]);
	}
	...
	int GetInstanceDiffMode(lua_State * L, Unit * ptr)
	{
		if(ptr == NULL || !ptr->IsInWorld())
			return 0;
		RET_NUMBER( ptr->GetMapMgr()->instance_difficulty );
		return 1; //returning 0 nr of values
	}

	int CastAllMapPlayers(lua_State * L, Unit * ptr)
	{
	...
	{
		EXTENSION_STORE_TYPE_NUMERIC	= 1,
	};

	class extension_numeric_store:public BaseObjectExtension
	{
	public:
		uint64 val;
	};

	int SetExtensionUInt64(lua_State * L, Unit * ptr)
	{
		...
		uint64 variable_val;
		if( lua_isnumber( L, 2 ) )
			variable_val = CHECK_ULONG(L,2);
		else
			variable_val = CHECK_GUID(L,2);

		extension_numeric_store *t = (extension_numeric_store *)ptr->GetExtension( variable_name );
		if( t == NULL)
		{
			t = new extension_numeric_store;
			t->type = EXTENSION_STORE_TYPE_NUMERIC;
			ptr->SetExtension( variable_name, t );
		}
		if( t->type == EXTENSION_STORE_TYPE_NUMERIC )
			t->val = variable_val;

		return 0;
	}

	int ModExtensionUInt64(lua_State * L, Unit * ptr)
	{
		const char *variable_name = luaL_checkstring(L, 1);
		if( variable_name == NULL )
			return 0;
		...
		extension_numeric_store *t = (extension_numeric_store*)ptr->GetExtension( variable_name );
		if( t != NULL && t->type == EXTENSION_STORE_TYPE_NUMERIC )
			t->val += variable_val;

		return 0;
	}

	int GetExtensionUInt64(lua_State * L, Unit * ptr)
	{
		const char *variable_name = luaL_checkstring(L, 1);
		if( variable_name == NULL )
			return 0;
		...
		if( t != NULL && t->type == EXTENSION_STORE_TYPE_NUMERIC )
		{
			//so that it can be processed as a number unless it is a GUID
			if( (t->val >> 32 ) == 0 )
	...
	int SetMapExtensionUInt64(lua_State * L, Unit * ptr)
	{
		...
		extension_numeric_store *t = (extension_numeric_store *)ptr->GetMapMgr()->GetExtension( variable_name );
		if( t == NULL)
		{
			t = new extension_numeric_store;
			t->type = EXTENSION_STORE_TYPE_NUMERIC;
	...
	int ModMapExtensionUInt64(lua_State * L, Unit * ptr)
	{
		if( ptr == NULL || ptr->GetMapMgr() == NULL )
			return 0;
		const char *variable_name = luaL_checkstring(L, 1);
	...
	int GetMapExtensionUInt64(lua_State * L, Unit * ptr)
	{
		if( ptr == NULL || ptr->GetMapMgr() == NULL )
			return 0;
	...
	int SetMapExtensionInt32(lua_State * L, Unit * ptr)
	{
		if( ptr == NULL || ptr->GetMapMgr() == NULL )
			return 0;
		const char *variable_name = luaL_checkstring(L, 1);
		if( variable_name == NULL )
			return 0;
		int32 variable_val = CHECK_INT(L,2);
		...
		if( t->type == EXTENSION_STORE_TYPE_NUMERIC )
			*((int32*)&t->val) = variable_val;

		return 0;
	}

	int ModMapExtensionInt32(lua_State * L, Unit * ptr)
	{
		if( ptr == NULL || ptr->GetMapMgr() == NULL )
			return 0;
		const char *variable_name = luaL_checkstring(L, 1);
	...
	int GetMapExtensionInt32(lua_State * L, Unit * ptr)
	{
		if( ptr == NULL || ptr->GetMapMgr() == NULL )
			return 0;
