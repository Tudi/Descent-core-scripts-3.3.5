...
bool Execute(uint32 i, Spell* pSpell)
{
    ...
    */
    ...
    if( !target->HasFlag(UNIT_FIELD_AURASTATE,AURASTATE_FLAG_HEALTH_BELOW_20_PCT)
	...
	// causing ${$m1+$AP*0.2} damage and converting each extra point of rage into $*10;F1 additional damage (up to a maximum cost of 30 rage).  Only usable on enemies that have less than 20% health.
    uint32 add_damage = base_dmg + pSpell->u_caster->GetAP() * 20 / 100 ;
	int32 rage_consume;
	if( pSpell->u_caster->GetUInt32Value(UNIT_FIELD_POWER2) > 300 )
		rage_consume = 300;
	else
		rage_consume = pSpell->u_caster->GetUInt32Value(UNIT_FIELD_POWER2);
	add_damage += rage_consume * pSpell->GetProto()->dmg_multiplier[0];   // rage is *10 always
//printf("!!!! %u, %u, %f\n",base_dmg,pSpell->u_caster->GetAP() * 20 / 100,rage_consume * pSpell->GetProto()->dmg_multiplier[0]);
	...
	// zero rage
    pSpell->u_caster->ModSignedInt32Value(UNIT_FIELD_POWER2, -rage_consume);
    return true;
}
