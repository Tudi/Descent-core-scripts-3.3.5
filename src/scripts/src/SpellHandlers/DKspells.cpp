   ...
   //constructor
    DancingRuneWeapon(Creature* pCreature) : CreatureAIScript(pCreature)
    {
		...
		if( owner )
		{
			...
			_unit->SetUInt32Value( UNIT_FIELD_POWER1 + POWER_TYPE_RUNIC_POWER, owner->GetUInt32Value( UNIT_FIELD_ATTACK_POWER ) );
		}	
 		stored_spells = 0;
...
#define TOWER_SUPPORT_BASE_RANGE 7
...
void HealthShield::Init(Unit *un, AIType at, MovementType mt)
{
   ...
   un->SetUInt32Value( UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_ATTACKABLE_2 | UNIT_FLAG_NOT_ATTACKABLE_9 | UNIT_FLAG_DEAD);
   un->SetUInt32Value( UNIT_FIELD_MAXHEALTH, 10000 );
